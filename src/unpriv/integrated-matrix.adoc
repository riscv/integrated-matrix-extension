...
For each tile index _m_, the first `S_blocks` elements are active, where
...
where M = N = VLEN ÷ (SEW × `LAMBDA`).  Because M × R = (VLEN ÷ (SEW × `LAMBDA`)) ×
(`LAMBDA` × SEW ÷ _pw_) = VLEN ÷ _pw_, the scale data fills exactly one register
...
row strides of length R, the first `S_blocks` positions are active and
positions [`S_blocks`, R) are padding.  The following options are listed from simplest to most
complex.
