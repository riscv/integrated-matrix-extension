...
Arguments `A` and `B` are the input matrices. The result C is accumulated
in vector registers `v16` through `v31`. Arguments `M`, `N`, and `K`
define the shape of the matrix multiply, following BLAS convention.

The `requested_lambda` argument is a semantic `LAMBDA` request in
{`0`, `1`, `2`, `4`, `8`, `16`, `32`, `64`}. After establishing `SEW=64`
and `LMUL=1`, the routine submits that request through
`__riscv_vsetlambda` and uses the returned selected value for all geometry,
loop-step, address, and dispatch calculations. A zero request invokes the
preserve-or-initialize rule. The value `L = VLEN/SEW` is derived from
...
<2> `requested_lambda` is assumed to be in the valid intrinsic domain and
    is submitted only after the required `SEW` has been established. A zero
    request is permitted and invokes the preserve-or-initialize rule. The
    returned `lambda` is the actual selected value after WARL processing and
    is used throughout the microkernel. A zero return indicates that no
    nonzero value could be selected for the configuration.
...
The example is adaptable: it accepts the supported nonzero `LAMBDA`
selected after WARL processing and dispatches using that returned value.
Software whose A and B panel packing or kernel contract requires an exact
nonzero `LAMBDA` cannot silently accept canonicalization. It must compare
the returned `lambda` with `requested_lambda` and select an appropriately
packed panel, another kernel, or a fallback when they differ. A zero request
intentionally invokes preserve-or-initialize behavior and therefore does
not name an exact nonzero value.
