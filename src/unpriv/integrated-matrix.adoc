...
In general, if `L` = (`VLEN`/`SEW`) is a perfect square (an even power of 2), then the architecturally permissible values of `LAMBDA` are {`L`^½^, `L`^½^ ÷ 2, `L`^½^ ÷ 4}.
If `L` = (`VLEN`/`SEW`) is two times a perfect square (an odd power of 2), then the architecturally permissible values of `LAMBDA` are {(`L` ÷ 2)^½^, (`L` ÷ 2)^½^ ÷ 2}.
The architecturally permissible values of `LAMBDA`, and corresponding values of `EMUL_C`, for all possible values of (`VLEN`/`SEW`) are shown in <<ime-emulc-table>>.
