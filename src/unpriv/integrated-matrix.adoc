...
The number of vector registers with input tiles is different in each case.
...
An additional A input register is then loaded to compute the second results accumulator
...
organized int a 4-by-4 array of single-register accumulator tiles.
...
Achieved performance at the thread level is also constrained by operation latency. That can be mitigated through simultaneous multi-threading.
...
(If the shape of the C tile being computed is `M` × `M`, then the largest multiplier array that can be used effectively is also `M` × `M`. However, it is always possible to use a smaller array, but that leads to lower performance.)

*Cacheline utilization of the tile loads.* `LMUL` extends each A and B row along the K dimension. An order-preserving tile load `vmtl` transfers `LAMBDA` × `LMUL` contiguous `SEW`-bit storage elements per row. For a widening instruction, each such storage element contains `W` logical input elements of width `SEW`/`W`, so the arithmetic depth is
...
bits. This row length determines how well each transfer maps onto a memory cacheline. For `SEW` = 32 and a 512-bit (64-byte) cacheline: configuration (a), with `LAMBDA` = 1, transfers at most 8 × 32 = 256 bits per row even at the maximum `LMUL` = 8, filling only half a cacheline; configuration (b), with `LAMBDA` = 2, reaches a full 512-bit line at `LMUL` = 8; and configuration (c), with `LAMBDA` = 4, reaches a full line already at `LMUL` = 4. A `LAMBDA` that is too small can thus leave `vmtl` unable to issue full-cacheline row transfers within the available `LMUL`, wasting bandwidth on partial lines. This gives a lower bound on the useful `LAMBDA` that is independent of, and may exceed, the minimum required for legality.
...
* `G` is a power of two;
* 1 ≤ `G` ≤ `LAMBDA`.  Since both `LAMBDA` and `G` are powers of two and `1 ≤ G ≤ `LAMBDA``, `G` divides `LAMBDA`.
...
the paired scale is +∞, multiplying it by a non-zero finite partial sum
...
and need not be initialised.  When `S_blocks` = R, the layout is fully packed with
no padding.
...
illegal-instruction exception and software must use another scale-
construction strategy. Omitting the instruction-lambda field and relying on
the dynamically selected `vtype.lambda` value is insufficient unless
software first establishes and verifies that value, because changing `SEW`
...
[NOTE]
====
Order preserving tile load/store with LMUL > 1 offers optimization opportunities. While vmtl/vmts are very similar to vector constant-stride segment operations, the segment sizes are potentially larger.  Matching cache line size with LAMBDA × LMUL × SEW allows for full cacheline transfers.
====
[#ime-vmtls-lmul]
...
// Pre-condition: EMUL_C is one of 1, 2, 4, or 8.
...
For values of matching accumulator element type, the following identities
hold bitwise:
...
Because EMUL_C = VLEN / (SEW × LAMBDA²), the accumulator register-group multiplier
varies across implementations with different values of VLEN and LAMBDA, even for a fixed SEW tied to an output type.
...
Runtime selection of the appropriate code path is then performed based on the result of `vsetvl` and computations of EMUL_C = VLEN / (SEW × LAMBDA²).
...
function fp_group_sum(i : int, j : int,
  ...
  match psm {
    ...
    PSM_DISCLOSED => {
      ...
      // (e.g. RVBNA) can normalise all terms together. The loop visits k in
