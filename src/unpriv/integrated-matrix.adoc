...
All six strategies below are specific to the E8M0 layout defined by this
version: each encoded scale is 8 bits, and each (`scale_A`, `scale_B`) pair
occupies one 16-bit paired-scale element.
...
With EEW = 16 and LMUL = 1, linesize = LAMBDA = R (when SEW = 16).  Each
...
This form requires SEW = 16 (i.e., R = LAMBDA).  For SEW > 16, R > LAMBDA
...
*Option 6 — Separate E8M0 scales via `vmtl.v` + `vzip.vv` (SEW = 16):*

When SEW = 16 and E8M0 scales reside in separate 2D arrays, `vmtl.v` can
load each scale array with the correct tile stride, then `vzip.vv` pairs them:
