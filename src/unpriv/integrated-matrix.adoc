...
* Lower byte [7:0]: `scale_A` — the block scale for a row of A~tile~.
* Upper byte [15:8]: `scale_B` — the block scale for a column of B~tile~^T^.
...
[#ime-block-scale-v0]
.Block scales are folded into `v0` in pairs, separated by the row stride R = `LAMBDA` × SEW / _pw_. Each block-scale pair supplies `scale_A` and `scale_B` for up to `block_size` consecutive logical input elements along the K dimension in the corresponding row of A~tile~ and column of B~tile~^T^; the final block may contain fewer elements. `block_size` is 32 when `bs=0` and 16 when `bs=1`. In matrix-tile representation, `v0` has the paired scales arranged in the correct scale rows. In a multi-lane setup, the scales are distributed across the lanes appropriately.
...
[#ime-mx-v0-lanes]
.Example for the use of microscales in a 4-lane setup for two blocks per row of A~tile~ and per column of B~tile~^T^. The red lines symbolize the split of the vector register group across the lanes. The R-strided scales distribution in `v0` leads to the correct lane-wise block scales layout.
...
For the currently defined E8M0 scale format, whose scale-pair width _pw_ is
16 bits, the active scale pairs fit within each row stride exactly when
...
The tile-strided physical scale layout in `v0` occupies M × R 16-bit
