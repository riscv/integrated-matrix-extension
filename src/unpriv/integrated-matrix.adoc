...
widening factor, `VL` is the current vector length, and `LAMBDA` is the
IM tile-layout parameter introduced below.

`EMUL_C` is the register-group multiplier for output C and is computed as:

    EMUL_C = (VLEN/SEW) / LAMBDA²

A, B, and C use row-major register layouts. A has shape `M` × `K_eff`.
The active B tile has shape `N` × `K_eff`, so its transposed view B^T^
has shape `K_eff` × `N`. The C register group has a fixed physical shape
`M` × `M`, while each instruction updates an active `M` × `N` sub-tile.

The dimensions are:

    M     = VLEN / (SEW × LAMBDA)
    K_eff = LAMBDA × W × LMUL
    N     = VL / (LAMBDA × LMUL), with 0 ≤ N ≤ M

At the maximum applicable `VL`, `N = M`; B then has shape
`M` × `K_eff`, and all columns of the physical C tile are active.

The complete physical and active tile geometry is defined in
<<integrated-matrix-active-tile-geometry>>.
...
The goal of the IM extension is to support the computation
C ← A × B^T^ + C, where A, B, and C are matrix tiles.
In this computation, A and B are _input tiles_ and C is the
_accumulator tile_. Input and accumulator tiles are stored in vector
register groups.

For an active computation, A has shape `M` × `K`, B has shape
`N` × `K`, B^T^ has shape `K` × `N`, and the active portion of C has
shape `M` × `N`. The C register group has a fixed physical shape
`M` × `M`, of which the first `N` columns are active. The current `VL`
selects `N`, with `0 ≤ N ≤ M`.

The geometry examples in this subsection illustrate the full-column case
`N = M`. In that case, both A and B have shape `M` × `K`, and all
columns of the square physical C tile participate.

For some (`VLEN`, `SEW`) configurations, the architecture permits
multiple tile geometries. The architecture defines the set of
architecturally permissible nonzero `LAMBDA` values, and each
implementation defines a supported subset of those values. Within the
IME-legal configuration domain, that supported subset shall be nonempty.

The current `LAMBDA` is a dynamically selected tile-layout parameter
drawn from the implementation-supported set. Software may request a value
through the architectural configuration mechanisms, subject to the WARL
selection rules defined in <<integrated-matrix-lambda>>.

Given the implementation-defined vector-register width `VLEN` and a
selected element width `SEW`, a single vector register contains
`VLEN`/`SEW` elements. `LAMBDA` specifies the width, in `SEW`-bit
elements, of each row fragment held in one vector register. The tile
height is then
...
In this case, (`VLEN`/`SEW`) = 16 and, therefore, the architecturally permissible values of `LAMBDA` are 1, 2, and 4.
...
.Geometry of matrix tiles fitting in a single vector register for `VLEN` = 512 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 16.) There are three architecturally permissible values of `LAMBDA`, with corresponding values of `M`. (a) `LAMBDA` = 1, `M` = 16. (b) `LAMBDA` = 2, `M` = 8. (c) `LAMBDA` = 4, `M` = 4.
...
In the full-column case, the A and B tiles that fit in a single vector register have shape `M` × `K`, while the fixed physical C tile has shape `M` × `M`; these shapes depend on `LAMBDA`.
...
[#ime-emulc-examples]
.Geometry of the computation of C ← A × B^T^ + C for `VLEN` = 512 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 16.) There are three architecturally permissible values of `LAMBDA`; each value affects the shapes of the A, B, and C tiles, including the multiplicity `EMUL_C` of the vector register group holding the accumulator tile C. (a) `LAMBDA` = 1, `M` = 16, `EMUL_C` = 16. (b) `LAMBDA` = 2, `M` = 8, `EMUL_C` = 4. (c) `LAMBDA` = 4, `M` = 4, `EMUL_C` = 1.
...
This indirect restriction arises because the 32 vector registers must hold tiles A, B, and C.
To leave enough registers for the input tiles A and B, we require `EMUL_C` ∈ {1, 2, 4, 8, 16}.
This imposes a minimum `LAMBDA` value when (`VLEN`/`SEW`) is large.
As an example, let us now consider the case `VLEN` = 1024 and `SEW` = 32, leading to (`VLEN`/`SEW`) = 32.
The two architecturally permissible values of `LAMBDA` (2 and 4), and the corresponding values of `M` (16 and 8), are illustrated in <<ime-vec-reg-geometry-L32>>.
A choice of `LAMBDA` = 1 would result in `EMUL_C` = 32, leaving no space for the A and B tiles.
The two possible configurations for `VLEN` = 1024 and `SEW` = 32 are shown in <<ime-emulc-examples-L32>>.

[#ime-vec-reg-geometry-L32]
.Geometry of matrix tiles fitting in a single vector register for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 32.) There are two architecturally permissible values of `LAMBDA`, with corresponding values of `M`. (a) `LAMBDA` = 2, `M` = 16. (b) `LAMBDA` = 4, `M` = 8.
image::png/ime-vec-reg-geometry-L32.png[width=50%, align=center, alt="Diagram of tile geometry for a single vector register."]

[#ime-emulc-examples-L32]
.Geometry of the computation of C ← A × B^T^ + C for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 32.) There are two architecturally permissible values of `LAMBDA`; each value affects the shapes of the A, B, and C tiles, including the multiplicity `EMUL_C` of the vector register group holding the accumulator tile C. (a) `LAMBDA` = 2, `M` = 16, `EMUL_C` = 8. (b) `LAMBDA` = 4, `M` = 8, `EMUL_C` = 2.
image::png/ime-emulc-examples-L32.png[width=80%, align=center, alt="Diagram of C ← A × B^T^ + C computation for different choices of `LAMBDA`."]

In general, let `L` = (`VLEN`/`SEW`), where `L` is a power of two
greater than or equal to 1.

If `L` is a perfect square (an even power of two), the architecturally
permissible values of `LAMBDA` are the members of

    {L½, L½ / 2, L½ / 4}

that are integer powers of two greater than or equal to 1. These values
correspond to `EMUL_C` ∈ {`1`, `4`, `16`}.

If `L` is two times a perfect square (an odd power of two), the
architecturally permissible values of `LAMBDA` are the members of

    {(L / 2)½, (L / 2)½ / 2}

that are integer powers of two greater than or equal to 1. These values
correspond to `EMUL_C` ∈ {`2`, `8`}.

Consequently, for `L` = `1`, `2`, `4`, and `8`, the permissible
`LAMBDA` sets are {`1`}, {`1`}, {`1`, `2`}, and {`1`, `2`},
respectively.

The architecturally permissible values of `LAMBDA`, and corresponding
values of `EMUL_C`, for all possible values of (`VLEN`/`SEW`) are shown
in <<ime-emulc-table>>.

[#ime-emulc-table]
.Architecturally permissible values of `LAMBDA` for different values of `L` = (`VLEN`/`SEW`). Each entry shows the resulting multiplicity `EMUL_C` of the vector register group holding tile C.
image::png/ime-emulc-table.png[width=80%, align=center, alt="Table of choices of `LAMBDA` as a function of (`VLEN`/`SEW`)."]

We see in <<ime-emulc-table>> that the majority of cases permit either two or three `LAMBDA` values.
Furthermore, in those cases the corresponding values for `EMUL_C` are either `EMUL_C` ∈ {1, 4, 16} or `EMUL_C` ∈ {2, 8}.
A high-performance matrix-multiply microkernel is often organized for a
particular value of `EMUL_C`, because `EMUL_C` determines the number and
alignment of vector registers occupied by each C accumulator tile. The
same microkernel organization can otherwise be reused across combinations
of `VLEN` and `LAMBDA` that produce that `EMUL_C`.

For the `EMUL_C` dimension alone, one performance-portable strategy is to
provide five specialized microkernel organizations, one for each
`EMUL_C` ∈ {`1`, `2`, `4`, `8`, `16`}, and select among them at run time.
This strategy can maintain efficient use of the sixteen registers devoted
to C accumulators across all five geometries.

Five specialized organizations are not required for functional
portability. A correct implementation may instead use one generalized
microkernel, or a smaller set of specialized microkernels together with a
generic fallback.

==== Example of performance-portable matrix multiply dispatch

<<microdgemm-example>> illustrates one performance-oriented strategy in
which a binary selects among five microkernel organizations according to
the value of `EMUL_C`. The example specializes exhaustively along the
`EMUL_C` dimension; it does not imply that functional portability requires
five separate implementations.

Arguments `A` and `B` point to input panels already packed for the nonzero
`LAMBDA` specified by `requested_lambda`. The result C is accumulated in
vector registers `v16` through `v31`. Arguments `M`, `N`, and `K` define
the shape of the matrix multiply, following BLAS convention.

The `requested_lambda` argument is the exact semantic `LAMBDA` value used
to pack both panels and must be in {`1`, `2`, `4`, `8`, `16`, `32`, `64`}.
After establishing `SEW=64` and `LMUL=1`, the routine submits that request
through `+__riscv_vsetlambda+` and checks that the returned selected value
equals `requested_lambda` before proceeding. It uses this verified value
for all geometry, loop-step, address, and dispatch calculations. A zero
request is not permitted by this microkernel because it does not specify a
packing geometry. The value `L = VLEN/SEW` is derived from
`+__riscv_ime_vlen()+` rather than supplied by the caller.

Although many concepts will only be introduced later, the example
illustrates the general approach to low-overhead runtime specialization.
Configuration, computation of `EMUL_C`, and selection of the code path are
performed only once per microkernel execution. Once the appropriate code
path is selected, the loop body contains only input loads (`vle64`
instructions), computations (`vfmmacc` instructions), and address updates.

[#microdgemm-example]
.Runtime specialization of a double-precision GEMM microkernel by `EMUL_C`
[source,cpp,linenums]
----
void microdgemm
(
    ...
    u32      M,
    u32      N,
    size_t   requested_lambda
)
{
    vsetvli(5, 0, 64, 1, true, true);                 // <1>

    assert(requested_lambda != 0);
    size_t lambda = __riscv_vsetlambda(requested_lambda);
    assert(lambda == requested_lambda);               // <2>

    size_t L = __riscv_ime_vlen() / 64;               // <3>
    assert(0 == L % (lambda * lambda));
    size_t emul_c = L / (lambda * lambda);
    assert(emul_c == 1 || emul_c == 2 || emul_c == 4 ||
           emul_c == 8 || emul_c == 16);              // <4>
    assert(0 == K % lambda);                           // <5>

    for (u32 r=16; r<32; r++) vxor.vv(r, r, r);       // <6>
...
----
<1> The double-precision microkernel first establishes `SEW=64` and
    `LMUL=1`. Because this operation may change `SEW`, it may replace a
    previously selected `LAMBDA`.
<2> `requested_lambda` is assumed to be a valid nonzero semantic `LAMBDA`
    value and is the value used to pack both input panels. It is submitted
    only after the required `SEW` has been established. The assertions reject
    a zero request and require the returned `lambda` to equal the requested
    value. A different selected value, including zero, does not satisfy the
    kernel's packing requirement.
<3> `L` is computed from the architectural VLEN after establishing
    `SEW=64`; it is not trusted as a caller-supplied geometry value.
<4> The assertions verify that the selected `LAMBDA` produces an integral
    physical accumulator geometry and that `emul_c` is one of the five
    architectural C register-group multiplicities.
<5> For simplicity, `K` is an integer multiple of the actual selected
    `lambda`.
<6> Vector registers `v16` through `v31` hold the output accumulators and
    are initialized to zero.
<7> With `EMUL_C=16`, one matrix instruction updates a single
    sixteen-register C tile.
<8> With `EMUL_C=8`, the sixteen output registers hold two C tiles.
<9> With `EMUL_C=4`, the output is decomposed into a 2-by-2 array of C
    tiles.
<10> With `EMUL_C=2`, the output is decomposed into a 4-by-2 array of C
    tiles.
<11> With `EMUL_C=1`, the output is decomposed into a 4-by-4 array of
    single-register C tiles.

The five-way dispatch selects a microkernel organization for the verified
packing geometry; it does not repack the input panels. Although
`__riscv_vsetlambda` may canonicalize an unsupported request, this kernel
cannot proceed with the existing panels when the returned `lambda` differs
from `requested_lambda`. Software must instead select compatible packed
panels, repack for a supported geometry, use a suitable fallback, or report
failure before executing the microkernel's input loads and computations.

The assertions express the preconditions of this illustrative code. In a
production implementation, these preconditions must also be enforced when
assertions are disabled, either by explicit runtime checks or by a caller
that has already established and verified them.

The geometries and register configurations for the five microkernel versions are illustrated in <<ime-portable-microkernel>>.
In all versions, 16 vector registers are used to store accumulator tiles.
The number of vector registers used for input tiles differs among the cases.
The overall shape of the result C is either square, if `EMUL_C` ∈ {1, 4, 16}, or rectangular, if `EMUL_C` ∈ {2, 8}.

[#ime-portable-microkernel]
.Geometry of the five microkernel variants. (a) `EMUL_C`=16, `LAMBDA`=(`VLEN`/`SEW`)^½^ ÷ 4. (b) `EMUL_C`=8, `LAMBDA`=((`VLEN`/`SEW`) ÷ 2)^½^ ÷ 2. (c) `EMUL_C`=4, `LAMBDA`=(`VLEN`/`SEW`)^½^ ÷ 2. (d) `EMUL_C`=2, `LAMBDA`=((`VLEN`/`SEW`) ÷ 2)^½^. (e) `EMUL_C`=1, `LAMBDA`=(`VLEN`/`SEW`)^½^.
image::png/ime-portable-microkernel.png[width=100%, align=center, alt="Diagram of geometry for five microkernels."]

In the `EMUL_C=16` specialization, the sixteen-register accumulator
occupies `v16` through `v31`, and each `vfmmacc.vv` updates the complete
output tile.

In the `EMUL_C=8` specialization, there are two eight-register accumulator tiles.
At each iteration of the microkernel, we load one register with A input and one register
with B input before computing one result accumulator tile.
An additional A input register is then loaded to compute the second accumulator
tile, while reusing the B input register.

The `EMUL_C=4` specialization divides the same output-register space into
a 2-by-2 array of four-register accumulator tiles. It therefore loads two
A tiles and two B tiles for each K step and issues four matrix
multiply-accumulate instructions.

The `EMUL_C=2` specialization divides the output-register space into
a 4-by-2 array of two-register accumulator tiles. It loads four A input tiles
and two B input tiles for each microkernel iteration. Eight matrix multiply-accumulate instructions are used to update the result.

Finally, in the `EMUL_C=1` specialization, the output-register space is
organized into a 4-by-4 array of single-register accumulator tiles.
Four A input tiles and four B input tiles are loaded in each iteration, and 16 matrix multiply-accumulate instructions are used to update the result.

[#integrated-matrix-intrinsic-microkernel]
==== Companion microkernel using IME and RVV intrinsics

<<microdgemm-intrinsics-example>> expresses the five microkernel
organizations in <<microdgemm-example>> using only C, IME intrinsics, and
ordinary RVV intrinsics. The original mixed assembly/intrinsic listing is
retained. Both listings use the same packed A/B panel layout, K-loop step,
input pointer advances, and accumulator-tile organization.

The SEW-qualified selector `__riscv_vsetlambda_e64`, defined in
<<integrated-matrix-sew-lambda-selection>>, returns a supported `LAMBDA`
for software dispatch. The returned value must equal the nonzero
`requested_lambda` used to pack A and B. It is not a persistent hardware
configuration on which subsequent intrinsic calls rely. Each computational
intrinsic determines its required geometry from its accumulator type and
the execution-time `VLEN`, as specified in
<<integrated-matrix-intrinsic-compute-geometry>>.

The additional argument `C` points to writable storage for `16 × L`
doubles, where `L = VLEN/64`. The kernel starts its accumulators at positive
zero and overwrites this workspace; it does not load or add an existing C
matrix. `C` receives a packed intermediate result, not the final application
matrix. Subsequent software must copy or unpack these tiles into that
matrix, applying any required final layout or accumulation operation.

Each physical accumulator tile has `T × T` elements, where `T = L/lambda`,
and occupies `EMUL_C × L` consecutive doubles in the workspace. Within a
tile, elements are row-major. Across tiles, the tile-row index varies
fastest: `c00`, `c10`, and so on, followed by `c01`, `c11`, and so on. This
matches the ascending accumulator-register-group order in
<<microdgemm-example>>, without assigning fixed registers to C variables.

.Intrinsic microkernel output organization
[cols="1,1,1,3", options="header"]
|===
|EMUL_C |Tile rows |Tile columns |Packed tile order
|16 |1 |1 |`c00` (stored as two `m8` halves)
|8 |2 |1 |`c00`, `c10`
|4 |2 |2 |`c00`, `c10`, `c01`, `c11`
|2 |4 |2 |`c00`, `c10`, `c20`, `c30`, then `c01` through `c31`
|1 |4 |4 |`c00` through `c30`, then `c01` through `c31`, and so on
|===

For the full-block interpretation of `M` and `N`, they equal the number of
tile rows times `T` and the number of tile columns times `T`, respectively.
The caller supplies panels packed for the execution-time `VLEN` and the
requested `LAMBDA`, with sufficient accessible storage for all illustrated
loads. This example does not handle arbitrary edge tiles. Its C workspace
must not overlap the input panels. As in the original listing, the
assertions express preconditions that production code must enforce even
when assertions are disabled.

The listing assumes that an IME-capable toolchain provides the IME types
and intrinsic declarations defined in <<integrated-matrix-intrinsics>> in
addition to the ordinary RVV declarations. Vector variables are individually
named; physical register allocation is left to the compiler.

[#microdgemm-intrinsics-example]
.Double-precision GEMM microkernel using IME and RVV intrinsics, with packed output
[source,c,linenums]
----
#include <assert.h>
#include <stddef.h>
...
/* Requires the IME types and intrinsics specified in this chapter. */
void microdgemm_intrinsics
(
    size_t        K,
    const double *A,
    const double *B,
...
{
    /* Both input panels were packed for this exact nonzero LAMBDA. */
    const size_t lambda = __riscv_vsetlambda_e64(requested_lambda);
    assert(lambda == requested_lambda);                                 // <1>

    const size_t L = __riscv_ime_vlen() / 64;
    assert(L != 0 && L % (lambda * lambda) == 0);
    const size_t emul_c = L / (lambda * lambda);
    assert(emul_c == 1 || emul_c == 2 || emul_c == 4 || emul_c == 8 || emul_c == 16);
    assert(K % lambda == 0);                                            // <2>

    /* C has space for 16 * L doubles in packed accumulator-tile order. */
    if (emul_c == 16) {
        const size_t stepA = M * lambda;
        const size_t stepB = N * lambda;
        vfloat64m8_t  zero8 = __riscv_vfmv_v_f_f64m8(0.0, 8 * L);
        vfloat64m16_t c00   = __riscv_ime_vpair_f64m16(zero8, zero8);   // <3>

        for (size_t k = 0; k < K; k += lambda) {
            vfloat64m1_t a0 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            vfloat64m1_t b0 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c00 = __riscv_vfmmacc_vv_f64m16(c00, a0, b0, L);
        }

        vfloat64m8_t lo = __riscv_ime_vunpairlo_f64m16(c00);
        vfloat64m8_t hi = __riscv_ime_vunpairhi_f64m16(c00);
        __riscv_vse64_v_f64m8(C        , lo, 8 * L);
        __riscv_vse64_v_f64m8(C + 8 * L, hi, 8 * L);                    // <5>
    } else if (emul_c == 8) {
        const size_t stepA = M/2 * lambda;
        const size_t stepB = N * lambda;
        vfloat64m8_t c00 = __riscv_vfmv_v_f_f64m8(0.0, 8 * L);          // <4>
        vfloat64m8_t c10 = __riscv_vfmv_v_f_f64m8(0.0, 8 * L);

        for (size_t k = 0; k < K; k += lambda) {
            vfloat64m1_t a0 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            vfloat64m1_t b0 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c00 = __riscv_vfmmacc_vv_f64m8(c00, a0, b0, L);
            vfloat64m1_t a1 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c10 = __riscv_vfmmacc_vv_f64m8(c10, a1, b0, L);
        }

        __riscv_vse64_v_f64m8(C        , c00, 8 * L);
        __riscv_vse64_v_f64m8(C + 8 * L, c10, 8 * L);
    } else if (emul_c == 4) {
        const size_t stepA = M/2 * lambda;
        const size_t stepB = N/2 * lambda;
        vfloat64m4_t c00 = __riscv_vfmv_v_f_f64m4(0.0, 4 * L);
        vfloat64m4_t c10 = __riscv_vfmv_v_f_f64m4(0.0, 4 * L);
        vfloat64m4_t c01 = __riscv_vfmv_v_f_f64m4(0.0, 4 * L);
        vfloat64m4_t c11 = __riscv_vfmv_v_f_f64m4(0.0, 4 * L);

        for (size_t k = 0; k < K; k += lambda) {
            vfloat64m1_t a0 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            vfloat64m1_t b0 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c00 = __riscv_vfmmacc_vv_f64m4(c00, a0, b0, L);
            vfloat64m1_t a1 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c10 = __riscv_vfmmacc_vv_f64m4(c10, a1, b0, L);
            vfloat64m1_t b1 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c01 = __riscv_vfmmacc_vv_f64m4(c01, a0, b1, L);
            c11 = __riscv_vfmmacc_vv_f64m4(c11, a1, b1, L);
        }

        __riscv_vse64_v_f64m4(C         , c00, 4 * L);
        __riscv_vse64_v_f64m4(C +  4 * L, c10, 4 * L);
        __riscv_vse64_v_f64m4(C +  8 * L, c01, 4 * L);
        __riscv_vse64_v_f64m4(C + 12 * L, c11, 4 * L);
    } else if (emul_c == 2) {
        const size_t stepA = M/4 * lambda;
        const size_t stepB = N/2 * lambda;
        vfloat64m2_t c00 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c10 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c20 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c30 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c01 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c11 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c21 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);
        vfloat64m2_t c31 = __riscv_vfmv_v_f_f64m2(0.0, 2 * L);

        for (size_t k = 0; k < K; k += lambda) {
            vfloat64m1_t a0 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            vfloat64m1_t b0 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c00 = __riscv_vfmmacc_vv_f64m2(c00, a0, b0, L);
            vfloat64m1_t a1 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c10 = __riscv_vfmmacc_vv_f64m2(c10, a1, b0, L);
            vfloat64m1_t a2 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c20 = __riscv_vfmmacc_vv_f64m2(c20, a2, b0, L);
            vfloat64m1_t a3 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c30 = __riscv_vfmmacc_vv_f64m2(c30, a3, b0, L);
            vfloat64m1_t b1 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c01 = __riscv_vfmmacc_vv_f64m2(c01, a0, b1, L);
            c11 = __riscv_vfmmacc_vv_f64m2(c11, a1, b1, L);
            c21 = __riscv_vfmmacc_vv_f64m2(c21, a2, b1, L);
            c31 = __riscv_vfmmacc_vv_f64m2(c31, a3, b1, L);
        }

        __riscv_vse64_v_f64m2(C         , c00, 2 * L);
        __riscv_vse64_v_f64m2(C +  2 * L, c10, 2 * L);
        __riscv_vse64_v_f64m2(C +  4 * L, c20, 2 * L);
        __riscv_vse64_v_f64m2(C +  6 * L, c30, 2 * L);
        __riscv_vse64_v_f64m2(C +  8 * L, c01, 2 * L);
        __riscv_vse64_v_f64m2(C + 10 * L, c11, 2 * L);
        __riscv_vse64_v_f64m2(C + 12 * L, c21, 2 * L);
        __riscv_vse64_v_f64m2(C + 14 * L, c31, 2 * L);
    } else if (emul_c == 1) {
        const size_t stepA = M/4 * lambda;
        const size_t stepB = N/4 * lambda;
        vfloat64m1_t c00 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c10 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c20 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c30 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c01 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c11 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c21 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c31 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c02 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c12 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c22 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c32 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c03 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c13 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c23 = __riscv_vfmv_v_f_f64m1(0.0, L);
        vfloat64m1_t c33 = __riscv_vfmv_v_f_f64m1(0.0, L);

        for (size_t k = 0; k < K; k += lambda) {
            vfloat64m1_t a0 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            vfloat64m1_t b0 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c00 = __riscv_vfmmacc_vv_f64m1(c00, a0, b0, L);
            vfloat64m1_t a1 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c10 = __riscv_vfmmacc_vv_f64m1(c10, a1, b0, L);
            vfloat64m1_t a2 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c20 = __riscv_vfmmacc_vv_f64m1(c20, a2, b0, L);
            vfloat64m1_t a3 = __riscv_vle64_v_f64m1(A, L);  A += stepA;
            c30 = __riscv_vfmmacc_vv_f64m1(c30, a3, b0, L);
            vfloat64m1_t b1 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c01 = __riscv_vfmmacc_vv_f64m1(c01, a0, b1, L);
            c11 = __riscv_vfmmacc_vv_f64m1(c11, a1, b1, L);
            c21 = __riscv_vfmmacc_vv_f64m1(c21, a2, b1, L);
            c31 = __riscv_vfmmacc_vv_f64m1(c31, a3, b1, L);
            vfloat64m1_t b2 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c02 = __riscv_vfmmacc_vv_f64m1(c02, a0, b2, L);
            c12 = __riscv_vfmmacc_vv_f64m1(c12, a1, b2, L);
            c22 = __riscv_vfmmacc_vv_f64m1(c22, a2, b2, L);
            c32 = __riscv_vfmmacc_vv_f64m1(c32, a3, b2, L);
            vfloat64m1_t b3 = __riscv_vle64_v_f64m1(B, L);  B += stepB;
            c03 = __riscv_vfmmacc_vv_f64m1(c03, a0, b3, L);
            c13 = __riscv_vfmmacc_vv_f64m1(c13, a1, b3, L);
            c23 = __riscv_vfmmacc_vv_f64m1(c23, a2, b3, L);
            c33 = __riscv_vfmmacc_vv_f64m1(c33, a3, b3, L);
        }

        __riscv_vse64_v_f64m1(C         , c00, L);
        __riscv_vse64_v_f64m1(C +  1 * L, c10, L);
        __riscv_vse64_v_f64m1(C +  2 * L, c20, L);
        __riscv_vse64_v_f64m1(C +  3 * L, c30, L);
        __riscv_vse64_v_f64m1(C +  4 * L, c01, L);
        __riscv_vse64_v_f64m1(C +  5 * L, c11, L);
        __riscv_vse64_v_f64m1(C +  6 * L, c21, L);
        __riscv_vse64_v_f64m1(C +  7 * L, c31, L);
        __riscv_vse64_v_f64m1(C +  8 * L, c02, L);
        __riscv_vse64_v_f64m1(C +  9 * L, c12, L);
        __riscv_vse64_v_f64m1(C + 10 * L, c22, L);
        __riscv_vse64_v_f64m1(C + 11 * L, c32, L);
        __riscv_vse64_v_f64m1(C + 12 * L, c03, L);
        __riscv_vse64_v_f64m1(C + 13 * L, c13, L);
        __riscv_vse64_v_f64m1(C + 14 * L, c23, L);
        __riscv_vse64_v_f64m1(C + 15 * L, c33, L);
    }
}
----
<1> The SEW-qualified selector evaluates support for `SEW=64` independently
    of the incoming vector configuration. The exact-match assertion enforces
    the input-panel packing contract; a different selected value cannot be
    used with these panels.
<2> `L` is the number of FP64 elements in one vector register. Loads of the
    `m1` A/B operands and all matrix operations use `vl=L`, so the full
    physical C columns are active. `EMUL_C` selects the typed branch; each
    matrix intrinsic supplies its own required `LAMBDA` through that type.
<3> An `m16` accumulator is constructed from two fully initialized `m8`
    values. No ordinary RVV `m16` initialization intrinsic is assumed.
    Pairing and unpairing use the value semantics defined in
    <<integrated-matrix-ime-m16-pairing>>; register moves may be eliminated
    when allocation and liveness permit, but this is not guaranteed.
<4> An ordinary RVV initialization or store of an `mX` accumulator uses
    `X × L` elements, not the matrix instruction's `vl=L`. This initializes
    or stores the entire physical accumulator tile. All lengths in this
    example are at most the relevant operation's VLMAX.
<5> The `m16` value is split into low and high `m8` halves before storing.
    Together the stores write `16 × L` doubles. The other branches write
    the same total number in the packed tile order described above.

The inner loops retain the original input reuse and issue the same matrix
operations for each K step. The stores are outside the loops. Actual
instruction scheduling, register allocation, configuration-instruction
placement, and elimination of redundant initialization or register moves
are compiler decisions; this listing does not promise the exact physical
register assignments or instruction counts of the assembly-oriented example.

==== Implementation support choices for tile geometry (`LAMBDA` and `EMUL_C`)

For a given (`VLEN`, `SEW`) configuration, the architectural legality
rules may permit multiple `LAMBDA` values and corresponding tile
geometries. An implementation chooses which of those permissible values
it supports, subject to the requirement that its supported set be nonempty
within the IME-legal domain.

The choice of supported geometries is not free: each supported `LAMBDA`
fixes the associated values of `M` and `EMUL_C` and therefore represents
a particular tradeoff among arithmetic throughput, silicon area and
power, and memory-system efficiency. Software establishes a current
selected `LAMBDA` from the supported set through the architectural
configuration mechanisms.

We summarize the forces at work using the three configurations of
<<ime-emulc-examples>> (`VLEN` = 512, `SEW` = 32, so
`L` = `VLEN`/`SEW` = 16).

*Compute intensity versus memory bandwidth.* Each update of the accumulator register group by a computational instruction (in linear algebra terms, a _rank_-`LAMBDA` update) loads the A and B operands — `M`×`LAMBDA` elements each — and performs `M`×`M`×`LAMBDA` multiply-accumulates. The compute intensity, in MACs per loaded element, is therefore

    I = (M×M×LAMBDA) ÷ (2×M×LAMBDA) = M ÷ 2 = L ÷ (2×LAMBDA).

For the three cases this gives `I` = 8, 4, and 2 MACs/load for <<ime-emulc-examples>> (a), (b), and (c) respectively: the larger the C-panel (larger `M`, smaller `LAMBDA`), the more arithmetic is extracted per byte moved, and the less vector register fetch bandwidth is needed to keep the array busy. In roofline terms the attainable throughput is

    P = min(P_peak, I × B),

where `B` is the delivered operand bandwidth and `P_peak` is the flat, compute-bound ceiling set by the number of instantiated MAC units. The product `I` × `B` is only the sloped, bandwidth-bound ceiling; a bandwidth-limited implementation raises its attainable `P` by choosing a geometry with higher `I` (smaller `LAMBDA`), which shifts the ridge point toward the arithmetic ceiling.

The analysis above applies to a single computational instruction.
For the microkernel in <<ime-portable-microkernel>>, the compute intensity is the same for each set of legal choices, since the shape of the result C is the same for all `EMUL_C` ∈ {1, 4, 16} or `EMUL_C` ∈ {2, 8}.
...
*Peak throughput versus area and power.* The number of MAC units a configuration can usefully instantiate scales with the C-panel area and the rank:
    ...
    N_MAC = M×M×LAMBDA = L² ÷ LAMBDA.

<<ime-emulc-examples>> (a) is a rank-1 outer-product engine of up to 16 × 16 = 256 MACs; <<ime-emulc-examples>> (c) is a rank-4 engine of 4 × 4 × 4 = 64 MACs. Both `N_MAC` and `I` increase as `LAMBDA` decreases, so a performance-only argument always favors the smallest architecturally permissible `LAMBDA` — the largest corresponding `EMUL_C` in <<ime-emulc-table>>. Area and power push the other way: the 256-MAC array of (a) is four times the arithmetic hardware of (c). An implementation with a limited area or power budget may therefore choose to support a larger `LAMBDA`, accepting lower compute intensity and lower peak throughput in exchange for a smaller multiplier. Subject to that budget, the highest-performance supported geometry for a given `VLEN` and `SEW` is the one with the largest `EMUL_C` among the implementation's supported values.
Thread-level performance is also constrained by operation latency, which can be mitigated through simultaneous multithreading.

*Dependence on `L`, and hence on `SEW`.* As shown in <<ime-emulc-examples>> and <<ime-emulc-examples-L32>>, different architecturally permissible values of `LAMBDA` for a given `L` = `VLEN`/`SEW` lead to different shapes of the multiplier array. Since `VLEN` is fixed for an implementation, support for appropriate `LAMBDA` values can let the same multiplier array shape be used for different `SEW`. For example, if `VLEN`=512, a 16 × 16 array can be used with both (`SEW` = 32, `LAMBDA` = 1) and (`SEW` = 16, `LAMBDA` = 2). In general, a `M` × `M` multiplier array can be used when the selected `LAMBDA` is `L` ÷ `M` and that value is supported. Narrower C elements (smaller `SEW`) raise the value of `L` and therefore require selecting a larger supported `LAMBDA` to obtain the same multiplier shape.
As previously discussed, a larger choice of `M`, and corresponding `EMUL_C`, leads to higher compute intensity.
(If the C tile has shape `M` × `M`, the largest multiplier array that can be used effectively is also `M` × `M`. A smaller array remains possible but reduces performance.)

*Cache-line utilization of the tile loads.* `LMUL` extends each A and B row along the K dimension. An order-preserving tile load `vmtl.v` transfers `LAMBDA` × `LMUL` contiguous `SEW`-bit storage elements per row. For a widening instruction, each such storage element contains `W` logical input elements of width `SEW`/`W`, so the arithmetic depth is

    K_eff = LAMBDA × W × LMUL.

The amount of storage transferred for each row is therefore

    LAMBDA × LMUL × SEW = K_eff × (SEW/W)

bits. This row length determines how well each transfer maps onto a memory cache line. For `SEW` = 32 and a 512-bit (64-byte) cache line: configuration (a), with `LAMBDA` = 1, transfers at most 8 × 32 = 256 bits per row even at the maximum `LMUL` = 8, filling only half a cache line; configuration (b), with `LAMBDA` = 2, reaches a full 512-bit line at `LMUL` = 8; and configuration (c), with `LAMBDA` = 4, reaches a full line already at `LMUL` = 4. A `LAMBDA` that is too small can thus leave `vmtl.v` unable to issue full-cache-line row transfers within the available `LMUL`, wasting bandwidth on partial lines. This gives a lower bound on the useful `LAMBDA` that is independent of, and may exceed, the minimum required for legality.

*Summary of the tradeoff.* `LAMBDA` (and thus `EMUL_C`) is selected to balance three competing objectives: high compute intensity and peak throughput, which favor small `LAMBDA` (large C-panel, large `EMUL_C`); bounded multiplier area and power, which favor large `LAMBDA`; and full-cache-line tile loads, which impose a minimum useful `LAMBDA`. The resulting choice must in all cases satisfy the legality constraints of <<ime-emulc-table>>, in particular `EMUL_C` ∈ {1, 2, 4, 8, 16} and the reservation of register groups for the A and B operands.

We have introduced the fundamental concepts related to the geometry of a C ← A × B^T^ + C computation.
...
[#ISA-control-geometry]
==== ISA control of matrix tile geometry

The geometry of the multiplier and the tiles is defined by the new `lambda` (the selected `LAMBDA`) field, which is encoded in 3 bits in the `vtype` CSR, in combination with the per-instruction widening factor `W` and the standard vector-configuration values `LMUL`, `SEW`, and `VLEN`.
...
* The _accumulator_ C is stored in a vector register group with element width `SEW`.
  ...
  `EMUL_C` = (`VLEN`/`SEW`) ÷ `LAMBDA`^2^, where `LAMBDA` is the tile-layout parameter decoded from the `lambda[2:0]` field in `vtype`.
  `LAMBDA`^2^ must divide (`VLEN`/`SEW`), giving `EMUL_C` ∈ {1, 2, 4, 8, 16}.
...
However, `EMUL_C` is derived from the tile geometry (`VLEN`, `SEW`, `LAMBDA`) rather than from an `EEW` / `SEW` ratio, and it is independent of `vtype.LMUL`.
...
For the logical input-element width `EEW_A = SEW/W`, each vector register
in an A or B register group contains `LAMBDA × W` consecutive logical K
elements from each represented tile row. The registers of the `LMUL`
group are concatenated in increasing register-number order along the K
dimension. Thus, for a logical element at row index `r` and K index `k`,
the register offset within the group is `k / (LAMBDA × W)`, and the
element offset within that register is
`r × (LAMBDA × W) + k % (LAMBDA × W)`, where `/` denotes integer
division and `%` denotes remainder. The `W` logical elements packed within
each `SEW`-wide storage position occupy consecutive K indices.

The C accumulator register group `vd` shall not overlap either input
register group `vs1` or `vs2`. Two register groups overlap when they name at
least one architectural vector register in common. This destination-overlap
restriction applies regardless of `VL`, including when `VL` = 0.

The two input register groups may overlap each other. Because both input
groups use the same `LMUL` and each base register is `LMUL`-aligned, this can
occur only when they use the same base register; partial overlap is not
possible.

[#ime-geometry-fig]
.Geometry of matrix tiles and element ordering for `VLEN`/`SEW` (vector length in C element width) = 32 and `LAMBDA`=4. Vector register groups are interpreted as 2D tiles. Vector element indices show the tile element order. (a) Non-widening case (W=1) with A, B, and C having the same SEW. (b) Double-widening case (W=2) with A and B at half the SEW of C, with two narrow input elements packed into each SEW-wide storage position.
...
The effective K-dimension of the multiplication (shared inner dimension of A and B^T^) is determined by `LAMBDA` from `vtype`, scaled by a per-instruction widening factor `W` and further multiplied by `LMUL`:

    K_eff = LAMBDA × W × LMUL.

`W` is 1 for non-widening instructions, 2 for 2× widening instructions, 4 for 4× widening instructions, and 8 for 8× widening instructions.
...
<<tile-dimensions>> shows how the tile dimensions follow from `VLEN`,
`SEW`, `VL`, `LAMBDA`, `W`, and `LMUL`.

[#tile-dimensions]
.Tile dimensions derived from the vector configuration
...
|K~eff~
|`LAMBDA` × `W` × `LMUL`
|Logical input elements along the shared K dimension.

|M
|(`VLEN` / `SEW`) / `LAMBDA`
|Rows of A and of the physical and active C tiles.

|N~max~
|M
|Maximum B rows and physical C columns.

|N
|`VL` / (`LAMBDA` × `LMUL`)
|Active B rows and active B^T^ and C columns; `0 ≤ N ≤ N~max~`.
|===
...
A~matrix~, B~matrix~, or
C~matrix~. A matrix may be stored in row-major,
...
A~tile~ has shape
M × K~eff~ and is stored in row-major register
layout. The in-register input tile B~tile~ has shape
N × K~eff~ and is also stored in row-major register
layout.

The operation forms dot products between rows of A~tile~
and rows of B~tile~; equivalently, it forms dot products
between rows of A~tile~ and columns of
B~tile~^T^.
Therefore,
it is often clearer to view the same B~tile~ register
contents as a transposed tile. This equivalent
view is denoted B~tile~^T^, has shape
K~eff~ × N, and is stored in column-major register
layout.
...
====
[.text-center]
B~tile~^T^[k,j] = B~tile~[j,k]
====

In the equations in this chapter, `SUM`~k=a~^b^ denotes summation over integer `k` from `a` through `b`, inclusive, and `SUM`~k∈S~ denotes summation over all `k` in the set `S`.
...
====
[.text-center]
C~tile~[i,j] ← C~tile~[i,j] + `SUM`~k=0~^K_eff−1^ A~tile~[i,k] × B~tile~^T^[k,j]
====
...
====
[.text-center]
C~tile~ ← C~tile~ + A~tile~ × B~tile~^T^
====

Consequently, if A~tile~ = B~tile~, the
operation computes:

[%unbreakable]
====
[.text-center]
C~tile~ ← C~tile~ + A~tile~ × A~tile~^T^
====
...
====
[.text-center]
C~matrix~ ← C~matrix~ + A~matrix~ × B~matrix~,
====

software loads or constructs in-register tiles such that the tile elements
represent the desired matrix panels. For an
M × K~eff~ panel of A~matrix~,
the natural relationship is:

[%unbreakable]
====
[.text-center]
A~tile~[i,k] = A~matrix~[i,k]
====

For a K~eff~ × N panel of
B~matrix~, the required relationship is:

[%unbreakable]
====
[.text-center]
B~tile~^T^[k,j] = B~matrix~[k,j]
====
...
====
[.text-center]
B~tile~[j,k] = B~matrix~[k,j].
====

Thus, the in-register B~tile~ object is a row-major
N × K~eff~ tile, while the arithmetic naturally uses
the equivalent column-major B~tile~^T^ view.

If B~matrix~ is stored in column-major order in memory,
its columns are contiguous and may be loaded directly into the
column-major B~tile~^T^ view using an order-preserving
tile load, as described in <<#zvvmtls>>. If
B~matrix~ is stored in row-major order, a transposing
...
supported set is nonempty there, so the selection yields a nonzero
value.
...
`SEW`) configuration only when it yields a valid C register group; that is,
when

`EMUL_C` = `VLEN` ÷ (`SEW` × `LAMBDA`²) = `M` ÷ `LAMBDA`

is an integer in {`1`, `2`, `4`, `8`, `16`}.

The _IME-legal configuration domain_ is the set of (`VLEN`,
`SEW`) for which at least one architecturally permissible nonzero lambda
exists. Note that lambda = `1` is permissible only when
`SEW ≤ VLEN ≤ 16 × SEW`; for `VLEN > 16 × SEW`, the minimum
permissible lambda is larger than `1`, and for `VLEN < SEW` no lambda is
permissible (the configuration lies outside the IME-legal domain).
...
that domain the set of supported nonzero lambda values is nonempty,
...
[#integrated-matrix-lambda,reftext="lambda write rules"]
...
Within the IME-legal domain the supported set is nonempty, so a `000`
...
A~tile~ and B~tile~. Both are stored
...
* `vs1`: Source vector register group containing the row-major architectural A matrix tile.
...
nonzero when such an instruction begins, an illegal-instruction exception
is raised.
...
[NOTE]
====
The exact product of two finite floating-point values with p~A~ and p~B~
significand bits has at most p~A~ + p~B~ significand bits. An
implementation may compute the product in any internal representation that
preserves the exact significand and provides sufficient exponent range for
the exact product.
...
When p~A~ + p~B~ ≤ p~C~, `fmt_C` has sufficient significand precision
to represent the exact product without loss of significand bits. This
condition alone does not guarantee that the exact product is representable
in `fmt_C`: the product may still lie outside the exponent range of
`fmt_C`, or may require rounding in its subnormal range.

The significand-width condition holds for all mixed-format widening
combinations. For example, E4M3 × E5M2 → FP16 satisfies
4 + 3 = 7 ≤ 11, and FP16 × BF16 → FP32 satisfies
11 + 8 = 19 ≤ 24. Extreme operand values may nevertheless produce exact
products that overflow, underflow, or require range-related rounding when
a later operation converts or rounds the result to `fmt_C`.

An implementation need not use a standard IEEE 754 type for the internal
product. Any representation that captures the exact product, such as an
integer significand-exponent pair, is sufficient, provided the overall
matrix multiply-accumulate result matches the semantics defined in
<<arithmetic-considerations>>.
====
...
The K-dimension, tile-dimension formulas, EMUL_C, and instruction-to-widening-factor mapping are the same as for the integer family (see <<zvvmm>>); the floating-point mnemonics are `vfmmacc.vv` (W=1), `vfwmmacc.vv` (W=2), `vfqmmacc.vv` (W=4), and `vf8wmmacc.vv` (W=8).
...
===== Arithmetic considerations for mixed-format inputs

Mixed-format computational instructions follow the rules in <<arithmetic-considerations>>.

[#arithmetic-considerations]
==== Arithmetic considerations

Each multiply-accumulate instruction computes, for every output element C~i,j~:

[%unbreakable]
====
[.text-center]
C~i,j~ ← C~i,j~ + `SUM`~k=0~^K_eff−1^ A~tile~[i,k] × B~tile~^T^[k,j]
====

where K~eff~ = `LAMBDA` × `W` × `LMUL`.
...
There are K~eff~ ÷ `W` = `LAMBDA` × `LMUL` sub-dot-products per output element.

When `W` = `1`, a sub-dot-product consists of a single product term.
That term is the exact mathematical product of the corresponding input values.

[#accumulation-and-rounding-model]
===== Accumulation and rounding model (floating-point)

The resulting value of element C~i,j~ for any given `LMUL` must match the value computed by applying a series of multiply-accumulate instructions with `LMUL=1`, in increasing order of vector-register indices.
Therefore, it is enough to specify the result of the computation

[%unbreakable]
====
[.text-center]
C~i,j~ ← C~i,j~ + `SUM`~k=0~^`LAMBDA`×W−1^ A~tile~[i,k] × B~tile~^T^[k,j]
====

An implementation partitions the `LAMBDA` sub-dot-products for each output element into consecutive groups of `G` sub-dot-products.
...
For unscaled floating-point multiply-accumulate instructions, every group within an `LMUL=1` step has exactly `G` sub-dot-products, and the number of groups in the computation of C~i,j~ above is `LAMBDA` ÷ `G`.
For microscaled instructions, additional microscaling block boundaries apply; see <<integrated-matrix-mx-shortened-groups>>.

`G` must satisfy:

* `G` is a power of two;
* `1 ≤ G ≤ LAMBDA`. Since both `LAMBDA` and `G` are powers of two, `G` divides `LAMBDA`.
...
** If `psm=1`, the partial sum `S` is formed using an
   _implementation-defined_ non-exact reduction of the group's products. The
   reduction receives the exact products in the canonical increasing-K order
   defined below and may be order-sensitive. It may use any internal
   representation chosen by the implementation. An implementation that
   supports `psm=1` for a given (`SEW`, `W`, `lambda`) shall publish, alongside
   its (`G`, `psm`, `rnd`) disclosure, the specific reduction algorithm and the
   values of any parameters it takes. Two implementations are bit-exactly
   compatible at `psm=1` only if both disclose the same algorithm, ordering
...
* After each group, the partial sum `S`, or the rounded value derived from `S`, is accumulated into the running value of C~i,j~ by computing

C~i,j~ ← round~frm~(C~i,j~ + S).

The accumulation rounding is performed with the rounding mode from `frm`.
If `rnd=xct`, the partial sum `S` is retained in sufficient internal precision so that the only rounding step in the update of C~i,j~ by that group is the final round~frm~(C~i,j~ + S).
...
and then accumulated into C~i,j~, the computation has two rounding
points: one when the partial sum is rounded, and another when the following
...
This two-stage rounding can differ from an accumulation performed with the
exact partial sum.
...
directly into C~i,j~, so the intermediate rounding point is
eliminated.
...
The values of `G`, `psm`, and `rnd` supported by an implementation may depend
on `SEW`, `W`, and `lambda`.
...
For every full or shortened group, the exact products supplied to
`fp_disclosed_reduce_group` are ordered by increasing logical K index, from
that group's `k_lo` through `k_hi`. When `W > 1`, the `W` products belonging
to each sub-dot-product therefore occupy consecutive positions in increasing
logical K order. A disclosed reduction may process this sequence directly or
may reassociate, pair, or reorder its products internally, but its
implementation-specific SAIL fragment shall completely define how the ordered
...
implementation accrues for every legal input. For an order-sensitive
reduction, this requirement includes all result and exception-flag behavior
that depends on the canonical input order or on any internally disclosed
reordering. The published fragment shall identify the version of this
specification against which it was authored.
...
chosen `psm=1` behavior.

The conformance and certification programs are the venue in which complete
machine-executable disclosure is checked. A certification program may
...
. *Direct integration with conformance testing.* The architectural model in this specification is itself SAIL-based; a published implementation fragment plugs into the same model with no glue code, so an architectural test suite can exercise the implementer's own definition of `fp_disclosed_reduce_group` and verify that it matches the hardware.
. *Reproducible cross-implementation comparison.* Two implementers disclosing nominally the same algorithm can settle disputes by diffing their SAIL fragments rather than re-deriving equivalence from prose.
...
of C is (`LAMBDA` × `LMUL`) ÷ `G`. Each group update includes one final
...
need not equal (`LAMBDA` × `LMUL`) ÷ `G`. In addition, when `rnd=frm` or
...
Implementations are free to declare any other `psm=1` reduction (for example, a Kulisch-style fixed-point accumulator, or a different bulk-normalized scheme with different parameter choices) provided they publish the algorithm and its parameters as required by the disclosure rule above.
====
...
In a *systolic-array* datapath, `G` is typically 1: each sub-dot-product is rounded and added to C immediately, yielding `LAMBDA` × LMUL rounding additions per output element.

In an *outer-product* datapath, `G` is typically on the order of `LAMBDA` (e.g., `LAMBDA`, `LAMBDA`÷2, `LAMBDA`÷4, or `LAMBDA`÷8): multiple sub-dot-products are accumulated at extended internal precision before a single rounding and C addition.
...
combined block scale (the product `scale_A` × `scale_B`)
...
* `decode_scale` decodes a scale-format value into the C accumulator format
  using the dynamic rounding mode `frm`. If that decoding requires conversion
  to the C accumulator format, the converted value and accrued floating-point
  exception flags are those defined by the scale format, the C accumulator
  format, and `frm`.
...
reduction. This definition includes any result or exception-flag behavior
that depends on product order. When composed with the shared SAIL-like
pseudocode of this specification, the implementation-specific fragment shall
reproduce both the hardware result bits and the hardware accrued `fflags`
value for every legal input.
...
.Block scaling multiplication example with SEW=16, `LAMBDA`=4, W=4, LMUL=1. Block scales are applied along rows of A~tile~ and columns of B~tile~^T^.
...
====
[.text-center]
C~i,j~ ← C~i,j~ + `SUM`~s=0~^S_blocks−1^ (`scale_A`~i,s~ × `scale_B`~j,s~ × (`SUM`~k∈block(s)~ A~tile~[i,k] × B~tile~^T^[k,j]))
====

where `S_blocks` = ⌈K_eff / block_size⌉ is the number of scale blocks
per row or column, and block(_s_) covers elements
[_s_ × block_size, min((_s_ + 1) × block_size, K_eff) − 1].

`scale_A`~i,s~ and `scale_B`~j,s~ are decoded from the
per-block scale fields in `v0` according to the scale format associated
with the input data type and converted to the C accumulator format using
`frm`. For each block, both scale conversions are performed before the
converted values are multiplied in `fmt_C` using `frm`; exception flags
from both conversions and the paired-scale multiplication accrue into
`fflags`.

If the combined block scale -- the product
`scale_A`~i,s~ × `scale_B`~j,s~ formed in the C
accumulator format -- is NaN, the corresponding output element
C~i,j~ is set to the default NaN and evaluation of that output element
terminates before this block's A×B products or accumulation are evaluated.
The scale blocks after the one that produced the NaN scale are not
evaluated, so they contribute no additional exception flags.

For E8M0, a combined block scale is NaN if either encoded scale is 0xFF,
or if conversion of finite scales to `fmt_C` followed by their `fmt_C`
multiplication produces NaN. The latter includes a zero-times-infinity
case when range conversion under `frm` produces zero for one scale and
...
`v0.scale` mechanism itself. All MX data types defined by this version of the
...
uses `frm`. The converted value and accrued exception flags are those of a
normal conversion to `fmt_C` under that rounding mode. Depending on
`fmt_C`, `frm`, and the scale value, range conversion may produce a finite
normal or subnormal value, +0, the largest finite positive value, or +∞.
For example, the E8M0 scale value 2^127^ cannot be represented as a finite
IEEE binary16 value and, depending on `frm`, converts to either +∞ or the
largest finite positive binary16 value. A sufficiently small positive E8M0
value may similarly convert to either +0 or the minimum positive subnormal
value. The applicable overflow, underflow, and inexact flags accrue as
defined by the conversion.

The paired-scale multiplication and subsequent block-scale operations
follow IEEE 754 rules. If one converted scale is +0 and the other is +∞,
the paired-scale multiplication produces the default NaN and raises the
invalid-operation flag, even though both encoded E8M0 scales are finite. If
the paired scale is +∞, multiplying it by a nonzero finite partial sum
yields a signed infinity, while multiplying it by zero produces the default
NaN. Implementations shall handle these cases as specified, not as
implementation-defined results.

[#integrated-matrix-mx-shortened-groups]
===== Arithmetic considerations and shortened groups

The computation of C~i,j~ follows the grouping, partial-sum,
...
register-index order. For `LMUL=1` step `step`, the scalar K interval is:
    ...
    step_k_lo = step × LAMBDA × W
    step_k_hi = step_k_lo + LAMBDA × W - 1
...
(`SEW`, `W`, `lambda`) entry. The exact products for each such group are
supplied in increasing logical K order. In particular, the disclosed SAIL
fragment shall neither assume that every microscaled group has length exactly
`G` nor leave any order-dependent behavior unspecified.

The partial sum `S` is then scaled by
`scale_A`~i,s~ × `scale_B`~j,s~, using the scale
...
All microscaling extensions defined by this version use the 8-bit E8M0
scale format. Accordingly, `v0` holds paired scale values at an effective
element width of 16 bits. Each 16-bit paired-scale element contains:

* Lower byte [7:0]: `scale_A` — the E8M0 block scale for a row of A~tile~.
* Upper byte [15:8]: `scale_B` — the E8M0 block scale for a column of B~tile~^T^.

Each 8-bit field has the E8M0 interpretation defined in
<<integrated-matrix-scale-formats>>.

[#ime-block-scale-v0]
.E8M0 block scales are folded into `v0` as 16-bit paired-scale elements, separated by the row stride R = `LAMBDA` × SEW / _pw_. Each paired-scale element supplies `scale_A` and `scale_B` for up to `block_size` consecutive logical input elements along the K dimension in the corresponding row of A~tile~ and column of B~tile~^T^; the final block may contain fewer elements. `block_size` is 32 when `bs=0` and 16 when `bs=1`. In matrix-tile representation, `v0` has the paired scales arranged in the correct scale rows. In a multi-lane setup, the scales are distributed across the lanes appropriately.
image::png/ime-mx-v0-format.png[width="80%", align="center"]

Let _sw_ be the width in bits of one encoded scale and _pw_ = 2 × _sw_
the width of a paired-scale element. For every scale format defined by this
version, _sw_ = 8 and _pw_ = 16.

A future extension may define an additional scale format. An additional
8-bit scale format may reuse this paired-scale layout only when explicitly
specified by that extension. A scale format wider or narrower than 8 bits
shall define its physical scale layout, row-stride and capacity rules,
instruction-legality rules, and software loading and packing conventions.
Support for such a format is not implied by the generic _sw_ and _pw_
notation used below.

The row stride in `v0` is R = `LAMBDA` × SEW ÷ _pw_ in units of _pw_-bit elements (see <<ime-block-scale-v0>>).

Within each physical row stride, positions _s_ with
`0 ≤ s < S_blocks` correspond to scale blocks, where

    S_blocks = ⌈K_eff / block_size⌉.

Element index _p_ = _m_ × R + _s_ addresses the scale pair for block
_s_.

For `scale_A`, index _m_ identifies row _m_ of
A~tile~. For `scale_B`, index _m_ identifies column
_m_ of B~tile~^T^. Specifically:

* `scale_A`~m,s~ is the lower _sw_ bits of the _pw_-bit element at index (m × R + s).

* `scale_B`~m,s~ is the upper _sw_ bits of the _pw_-bit element at index (m × R + s).

The paired-scale layout is provisioned for the fixed physical tile
dimensions. It contains `M` row strides of `R` _pw_-bit elements, where
`N_max` is the maximum B~tile~^T^/C column count and

    M = N_max = VLEN / (SEW × LAMBDA).

For an operation with active column count `N`, only the `scale_B` fields
whose row-stride index _m_ satisfies `0 ≤ m < N` are active. The `scale_B`
fields with `N ≤ m < M` are ignored and do not affect the result or
`fflags`. The physical layout and row stride are not compacted or otherwise
changed when `N < M`.
...
the physical paired-scale layout occupies exactly one vector register,
independent of the active value of `N`. Some fields may be inactive because
of partial columns or padding.

[#ime-mx-v0-lanes]
.Example of the E8M0 8-bit-scale/16-bit-pair layout in a 4-lane setup for two blocks per row of A~tile~ and per column of B~tile~^T^. The red lines symbolize the split of the vector register group across the lanes. The R-strided scale distribution in `v0` leads to the correct lane-wise block-scale layout.
image::png/ime-mx-v0-lanes.png[width="69%", align="center"]

**Physical-layout capacity proof:**

M × R = (VLEN / (SEW × LAMBDA)) × (LAMBDA × SEW / _pw_) = VLEN / _pw_.

Each of the VLEN / _pw_ positions is a _pw_-bit element, so the total is
exactly VLEN bits — the physical paired-scale layout occupies exactly one
vector register in every legal configuration.

**R ≥ S_blocks proof** (tile stride is never smaller than the active count):
...
* block_size = 32: The encoding tables (<<tbl-fp-encoding-map>>,
  ...
  Therefore K_eff = LAMBDA × W × LMUL ≤ 2 × LAMBDA × SEW, giving
  S_blocks = ⌈K_eff / 32⌉ ≤ ⌈2 × LAMBDA × SEW / 32⌉ ≤ LAMBDA × SEW / _pw_ = R
  (with _pw_ = 16 in this version).
* block_size = 16: The BS=16 legality check (W × LMUL ≤ SEW, stated in
  each instruction's Exceptions section) ensures
  K_eff = LAMBDA × W × LMUL ≤ LAMBDA × SEW, giving
  S_blocks = ⌈K_eff / 16⌉ ≤ ⌈LAMBDA × SEW / 16⌉ ≤ LAMBDA × SEW / _pw_ = R
  (with _pw_ = 16 in this version).

When `S_blocks` < R, positions _s_ ∈ [`S_blocks`, R) within each row are padding and their
contents are ignored by the instruction.

.E8M0 scale-layout parameters (block_size = 32, VLEN = 256)
[cols="1,1,1,1,1,1,1,1", options="header"]
|===
| SEW | LAMBDA | W | LMUL | M | S_blocks | R | 16-bit extent (M×R)
...
| 16 | 1 | 2 | 1 | 16 | 1 | 1 | 16
| 16 | 1 | 4 | 1 | 16 | 1 | 1 | 16
| 16 | 1 | 4 | 8 | 16 | 1 | 1 | 16 ^footnote:packfull[S_blocks = R: fully packed, no padding]
| 16 | 2 | 4 | 1 | 8  | 1 | 2 | 16
...
Within each row stride, elements at positions _s_ ∈ [`S_blocks`, R) are padding
and need not be initialized. When `S_blocks` = R, the layout is fully packed with
no padding.
...
Let `S_blocks_32` and `S_blocks_16` denote the numbers of active scale
blocks for block sizes 32 and 16, respectively:

    S_blocks_32 = ⌈K_eff / 32⌉
    S_blocks_16 = ⌈K_eff / 16⌉

Because every legal `K_eff` is a positive power of two,
`S_blocks_16 = S_blocks_32 = 1` when `K_eff ≤ 16`, and
`S_blocks_16 = 2 × S_blocks_32` when `K_eff ≥ 32`.

Selecting block size 16 therefore either leaves the active scale-pair count
per row or column unchanged or doubles it. It does not change the physical
`M × R` layout in `v0`; it may increase the number of active positions
within that fixed layout.

For the E8M0 scale format defined by this version, whose scale-pair width
_pw_ is 16 bits, the active scale pairs fit within each row stride exactly when

    W × LMUL ≤ SEW

Because IME supports `LMUL` ∈ {`1`, `2`, `4`, `8`}, the maximum `LMUL`
permitted by this block-size-16 capacity rule is the largest member of that
set no greater than `SEW/W`. If the condition is violated when `bs=1` and
`vm=0`, the instruction raises an illegal-instruction exception.

.Maximum LMUL imposed by block size 16 for currently defined MX encodings
[cols="3,2,1,1,1", options="header"]
|===
| Input → accumulator format | Instruction | W | SEW | Maximum LMUL
...
| All other currently defined MX encodings
| —
| —
| —
| 8 (no additional block-size-16 restriction)
|===

This table summarizes only the additional `block_size=16` row-capacity
restriction; all other instruction legality requirements continue to apply.
...
Microscaling requires SEW × LAMBDA ≥ _pw_ (the scale-pair width: 2 × _sw_) to
ensure that the paired scale elements fit within `v0`.
For E8M0 (_sw_ = 8, _pw_ = 16) this reduces to SEW × LAMBDA ≥ 16.
Configurations where SEW × LAMBDA < _pw_ with `vm=0` raise an
illegal-instruction exception.

Microscaling is particularly useful with narrow input formats (OFP4, OFP8),
whose limited per-element dynamic range is compensated by the block-level
E8M0 exponent, enabling high-throughput inference on microscaling-quantized
models.

===== Loading or constructing E8M0 paired scales
[NOTE]
====
This section is _informative_ (non-normative).

The five principal strategies below are specific to the E8M0 layout defined
by this version: each encoded scale is 8 bits, and each (`scale_A`,
`scale_B`) pair occupies one 16-bit paired-scale element.

The tile-strided physical E8M0 scale layout in `v0` occupies M × R 16-bit
elements (always VLEN ÷ 16 elements, exactly one vector register). Within
each of the M row strides of length R, positions _s_ with
`0 ≤ s < S_blocks` contain scale fields for defined blocks, and positions
[`S_blocks`, R) are padding.

For an operation with `N < M`, all M lower-field scale rows are available
for `scale_A`, but only the upper-field `scale_B` entries in row strides
`0` through `N−1` are active. The `scale_B` entries in row strides `N`
through `M−1` are padding and are ignored by the IME instruction. The
construction sequences below materialize and transfer the fixed M-row
physical layout. Any padding storage that they load must still be
addressable, even though its values do not affect the result or `fflags`.

The sequences below temporarily modify `vl` and one or more standard
`vtype` fields. A `vsetvli` or `vsetivli` that changes `SEW` may also cause
the selected `LAMBDA` to be replaced according to the preserve-or-initialize
rules in <<integrated-matrix-vtype-config>>.

Before executing the microscaled matrix instruction, software must
reestablish the complete intended compute configuration, including the
selected `LAMBDA`. Merely restoring `SEW` and `LMUL` with `vsetvli` does not
necessarily restore the previous `LAMBDA`. Software may, for example, save
`vl` and the complete `vtype` before constructing the scales and restore them
afterward with `vsetvl`. Alternatively, it may reestablish the configuration
fields individually and verify the resulting selected `LAMBDA`.

The following options are listed from simplest to most complex.

*Option 1 — Pre-paired E8M0 tile-strided load via `vmtl.v` (single instruction):*

When the application stores pre-paired E8M0 (`scale_A`, `scale_B`) 16-bit
elements in a 2D scale matrix (M rows, stride LD ≥ R between rows), a single
`vmtl.v` loads the current tile's scales directly into `v0`.

This option assumes that the intended compute configuration uses `SEW=16`
and that its selected `LAMBDA` has already been established. The temporary
configuration below retains `SEW=16` and changes `LMUL` to 1. Because the set
of supported `LAMBDA` values does not depend on `LMUL`, the selected
`LAMBDA` is retained.
...
With EEW = 16 and LMUL = 1, linesize = LAMBDA = R. Each `vmtl.v` line
corresponds to one scale row, and LD allows rows to be non-contiguous in the
outer scale matrix. When LD = R, this reduces to a unit-stride load.

For other intended compute `SEW` values, the relation R = LAMBDA does not
hold; use Option 2 unless a separate geometry-specific tile-load sequence
has been validated. Before the matrix instruction, restore the complete
intended compute configuration as described above.

*Option 2 — Pre-paired E8M0 flat load (`vle16.v`):*

If the pre-paired E8M0 tile-strided data is already contiguous in memory (M × R
...
This works for all `SEW` values because M × R = VLEN / _pw_ for every
configuration, where _pw_ is the scale-pair width.

*Options 3–5 — Constructing E8M0 pairs from separate `scale_A` / `scale_B` arrays:*

The following sequences construct the E8M0 paired layout from separate
`scale_A` and `scale_B` byte arrays, producing M × `S_blocks` contiguous pairs.
This produces the correct tile-strided layout when `S_blocks` = R (fully packed).
When `S_blocks` < R, the output must be placed at stride R, e.g., by storing
the M × `S_blocks` pairs into a pre-zeroed M × R buffer before loading with
Option 2.

*Option 3 — E8M0 packing with Base V (widen + shift + OR):*

[source,asm]
----
    # a0 = &scale_A, a1 = &scale_B (M × S_blocks bytes each), a2 = M × S_blocks
...
*Option 4 — E8M0 packing with Zvbb (widening shift + OR):*
...
*Option 5 — E8M0 packing with Zvzip (interleave):*
...
.E8M0 scale-packing instruction count comparison
...
The counts above cover only the local scale-loading or scale-packing
sequence. They exclude instructions needed to save, restore, or otherwise
reestablish the surrounding compute configuration. When `S_blocks` < R,
they also exclude the work needed to materialize the R-strided physical
layout.

*Target-specific optimization — separate strided E8M0 scale arrays via `vmtl.v` + `vzip.vv`:*

A target-specific implementation may load separate strided `scale_A` and
`scale_B` arrays with `vmtl.v` and then pair the results with `vzip.vv`.
This optimization is not generally portable across implementations and is
not included in the comparison table above.

For an intended `SEW=16` compute configuration, R equals the compute-time
selected `LAMBDA`. The temporary `SEW=8`, `LMUL=1` tile loads must therefore
use an effective `LAMBDA` exactly equal to that compute-time value. The
required value must be supported for the temporary (`VLEN`, `SEW=8`)
configuration.

A geometry-specialized sequence should encode the exact value in the
nonzero instruction-lambda field. In the template below, `LLAMBDA` denotes
one of `L1`, `L2`, `L4`, `L8`, `L16`, `L32`, or `L64`, and its value equals
the compute-time selected `LAMBDA` and R. An instantiated sequence replaces
`LLAMBDA` with the corresponding concrete assembler token; for example,
compute-time `LAMBDA=4` uses `L4`.

[source,asm]
----
    # a0 = &scale_A, a1 = &scale_B (M rows, LD bytes between rows)
    # a2 = M × R (= VLEN / 16), a3 = LD (leading dimension in bytes)
    # LLAMBDA encodes the exact compute-time LAMBDA (= R).
    vsetvli  t0, a2, e8, m1, ta, ma
    vmtl.v   v1, (a0), a3, LLAMBDA  # exact-immediate tile load of scale_A
    vmtl.v   v2, (a1), a3, LLAMBDA  # exact-immediate tile load of scale_B
    vsetvli  t0, a2, e8, mf2, ta, ma
    vzip.vv  v0, v1, v2              # pair into 16-bit layout
----

The nonzero instruction-lambda field is checked exactly and is not
WARL-canonicalized, as specified in
<<integrated-matrix-tile-ls-legality>>. If the required value is unsupported
for the temporary configuration, a tile load using that immediate raises an
illegal-instruction exception, and software must use another
scale-construction strategy. Omitting the instruction-lambda field and
relying on the dynamically selected `vtype.lambda` value is insufficient
unless software first establishes and verifies that value, because changing `SEW`
may replace it and silently change the memory traversal performed by
`vmtl.v`.

After this target-specific sequence, software must restore or reestablish
the complete intended compute configuration, including its selected
`LAMBDA`, before executing the microscaled matrix instruction.
====
...
<<tbl-mx-extensions>>.
All microscaling extensions defined by this version use the E8M0 scale format.
...
The tile dimensions are fully determined by the current `vtype` settings (SEW, VL, LMUL, and LAMBDA), consistent with the operand geometry of the multiply-accumulate instructions.
The tile load and store instructions make use of the following parameters from the CSRs:

* SEW — element width
* VL — active vector length (must be a multiple of LAMBDA × LMUL, the line size)
* LMUL — vector length multiplier
* LAMBDA — selected lambda, read from `lambda[2:0]` in `vtype`
...
.Loading a matrix tile from memory for LMUL=1. The matrix is laid out linearly in memory; the leading dimension `LD` specifies its row size in (a) or column size in (b). Element indices represent element offsets in memory. Blue arrows indicate the data ordering in memory and in the vector register group.
...
* For the order-preserving load/store (`vmtl.v`, `vmts.v`), LD = LAMBDA × LMUL
  — the in-memory layout, with elements contiguous to each other, matches
  the layout of the register group being loaded/stored.

* For the transposing load/store (`vmttl.v`, `vmtts.v`), LD = VLEN ÷ (SEW × LAMBDA)
  (= `M`, the tile edge) — the leading dimension of a matrix tile that
...
Efficient implementations of tile loads with `rs2 = x0` are essential to high-performance linear algebra kernels.
====
...
complete columns of B~tile~^T^. Each active column
of B~tile~^T^ contributes to the corresponding active
column of C. The impact is sketched in <<ime-load-store-vl>>.

[#ime-load-store-vl]
.Examples of element distribution for full and reduced vector-length matrix-tile load/store operations.
...
* An _immediate lambda_ `LLAMBDA` (one of `L1`, `L2`, `L4`, `L8`, `L16`, `L32`, `L64`), which overrides `lambda[2:0]` from `vtype` for the duration of that instruction.
...
[NOTE]
====
Order-preserving tile loads/stores with `LMUL > 1` offer optimization opportunities. Although `vmtl.v` and `vmts.v` are similar to vector constant-stride segment operations, their segment sizes can be larger. Matching the cache-line size to `LAMBDA × LMUL × SEW` enables full-cache-line transfers.
====
[#ime-vmtls-lmul]
.Order-preserving tile loads/stores with LMUL > 1 for row-major and column-major memory layouts.
...
[#ime-vmttl-fig]
.Transposing load examples with LMUL = 1 (left) and LMUL = 2 (right). Indices label SEW-bit elements in the row-major matrix in memory. Light-blue arrows indicate the memory/vector-register access order.
image::png/ime-vmttl-lmul.png[width=100%, align=center, alt="Depiction of transposing load with vmttl.v for LMUL=1 and LMUL=2."]
...
===== `vmtts.v` — Transposing Tile Store
    ...
    vmtts.v vs3, (rs1), rs2 [, LLAMBDA] [, vm]
...
The tile dimensions are fully determined by the current vector configuration:
    ...
    M_tile     = VLEN / (SEW × LAMBDA)       (rows of A and C per computation)
    K_eff      = LAMBDA × W × LMUL           (shared K-dimension step)
    N_tile     = VL / (LAMBDA × LMUL)        (active columns of B_tile^T and C; set via vsetvl)
    N_tile_max = M_tile                 (maximum columns when VL = VL_max)
    EMUL_C      = VLEN / (SEW × LAMBDA²)      (C accumulator register group size)

`M_tile` is fixed by the hardware (`VLEN`) and the chosen `SEW` and `LAMBDA`; it cannot be changed by `VL`.
`N_tile` is controlled by software through `VL`; use `vsetvl` to select the largest `N_tile` that fits the remaining columns.
`VL` (set via `vset{i}vl{i}` at `vtype.SEW` equal to the C element width) must be a multiple of LAMBDA × LMUL, so only full columns can be selected.
...
Thus the physical row-major position of C element C~i,j~ within
...
rows of A independent of `VL`, loading an A tile requires:
    ...
    VL = M_tile × LAMBDA × LMUL

which is the natural maximum `VL` for the chosen `LMUL`.

The B~tile~ load and computation use the narrower
active-column `VL`:

    VL = LAMBDA × LMUL × N_tile

The compute `VL` selects the number of active columns of
B~tile~^T^ and, equivalently, the number of active
...
When `N_tile` < `N_tile_max`, the elements C~i,j~ for
`0 ≤ i < M_tile` and `N_tile ≤ j < N_tile_max` are _C tile tail
elements_. C tile tail elements do not participate in the
multiply-accumulate computation, and their pre-instruction values are not
read as arithmetic inputs.
...
* If `vta` = 1 (tail-agnostic), each C tile tail element may independently
  retain its pre-instruction value or be overwritten with an all-ones
  `SEW`-bit pattern. The choice may vary independently among C tile tail
  elements and need not be deterministic.

If `VL` = 0, no C element is updated, regardless of `vta`.

[NOTE]
====
The architectural requirements above do not prescribe an implementation
mechanism. An implementation may preserve C tile tail elements by
suppressing writes, retaining an old physical-register mapping, performing
a read-merge, or using any other mechanism that produces the required
architectural result.

No C tile tail value contributes to an active C result. This permits
hardware implementations, including outer-product engines and
register-renaming machines, to process only the `N_tile` active columns of
the C tile without using the inactive C tile columns as arithmetic inputs.
====
...
With this configuration:
    ...
    linesize = LAMBDA × EMUL_C = N_tile_max
...
[NOTE]
====
The compute `VL = LAMBDA × LMUL × N_tile` used by a multiply-accumulate
...
[#integrated-matrix-m16-c-load-store,reftext="C tile load/store for EMUL_C = 16"]
[NOTE]
====
...
For clarity, this example covers configurations with
`EMUL_C` ∈ {`1`, `2`, `4`, `8`}. The legal `EMUL_C = 16` case uses an
`m16` accumulator and requires the separate two-half C load/store sequence
described in <<integrated-matrix-m16-c-load-store>>.
The configuration uses SEW=32, LMUL=1, and `LAMBDA`=4, giving K_eff = 4 and
M_tile = VLEN/128.
...
//
// Precondition: vtype can be configured for SEW=32, LMUL=1, lambda=4.
// Precondition: EMUL_C is one of 1, 2, 4, or 8.
// Precondition: K is a multiple of K_eff (= lambda × LMUL = 4).
...
for (j = 0; j < N; j += N_tile_max) {
    // Select the active number of B/C columns explicitly. Full-width
    // tiles are followed by at most one partial tile.
    const int N_tile = min(N - j, N_tile_max);

    // VL counts SEW-wide storage elements. Each active B/C column
    // contributes lambda * LMUL_AB such elements. Because N_tile is no
    // greater than N_tile_max, vl_compute is no greater than VLMAX, so
    // vsetvl shall return exactly the requested value.
    const int vl_compute = lambda * LMUL_AB * N_tile;

    configure_vtype(SEW=32, LMUL=1, lambda=4);
    const int vl_actual = vsetvl_e32m1(vl_compute);
    assert(vl_actual == vl_compute);
...
* The A tile must be loaded with `VL = M_tile × LAMBDA × LMUL` (the maximum VL
...
* The order-preserving `B_matrix` load constructs
  B~tile~^T^ directly. The load and the
  multiply-accumulate instruction share
  `VL = LAMBDA × LMUL × N_tile`, which selects the active columns of
  B~tile~^T^ and C.
...
* This simplified example uses `v8` as the C accumulator base and therefore
  covers `EMUL_C` values through 8. For `EMUL_C = 16`, the `m16` accumulator
  group shall be placed at `v16`–`v31` when partial-column masks are
  required, and its C loads and stores are performed as two `LMUL=8` halves,
  as described in <<integrated-matrix-m16-c-load-store>>.
...
encoded in the accumulator type suffix is `EMUL_C` = `VLEN` ÷ (`SEW` × `LAMBDA`²), which is determined by `VLEN`, `SEW`,
...
[#integrated-matrix-intrinsic-compute-geometry]
===== Geometry of computational intrinsics

For an IME multiply-accumulate intrinsic, the accumulator element width
defines `SEW`, and the accumulator register-group multiplier defines
`EMUL_C`. Together with the execution-time `VLEN`, these determine the
required positive `LAMBDA` through

    EMUL_C = VLEN / (SEW × LAMBDA²).

The required value shall be an encodable, architecturally permissible,
implementation-supported nonzero `LAMBDA`. All other legality requirements
for the operation also apply. The compiler shall arrange for each operation
to execute with that geometry. The intrinsic does not depend on a selected
`LAMBDA` left by a preceding configuration intrinsic or vector operation,
and a different geometry shall not be silently substituted because a
configuration write canonicalizes an unsupported request.

This rule applies to long-form and overloaded computational intrinsics,
including microscaled forms. The A/B types and any `_lm{N}` qualifier
continue to specify the input `LMUL`; they do not change the `LAMBDA`
required by the accumulator type. The same intrinsic type combination may
require different `LAMBDA` values on implementations with different `VLEN`.

Software may use the SEW-qualified selectors in
<<integrated-matrix-sew-lambda-selection>> to choose a supported typed
specialization. A selector call is not required before every computational
intrinsic, nor does this rule require a support-probing sequence before every
operation. It is the program's responsibility to execute only supported
specializations; the compiler may reuse a suitable hardware configuration
whenever doing so preserves the intrinsic's semantics.

This inference is specific to computational intrinsics. The register-group
multiplier in a tile load/store intrinsic describes its `LMUL`, not an
accumulator `EMUL_C`; the tile load/store lambda-selection rules remain
separate and are not changed by this paragraph.
...
do not correspond to new ISA instructions. Their value semantics and
expected lowering are given below.
...
For an `m16` value held in a sixteen-register group, the low half
corresponds to the first eight architectural vector registers of the group
(register offsets 0 through 7), and the high half corresponds to the final
eight registers (offsets 8 through 15).

The pair and unpair operations have pure value semantics:

* `__riscv_ime_vunpairlo_*m16(src)` returns an `m8` value bitwise equal
  to the low half of `src`, and `__riscv_ime_vunpairhi_*m16(src)` returns
  an `m8` value bitwise equal to the high half. The `m16` source value is
  unchanged. The returned C-language value does not alias the source
  C-language value. Subsequent operations on the returned value do not modify
  `src`.

* `__riscv_ime_vpair_*m16(lo, hi)` returns an `m16` accumulator value
  whose low and high halves are bitwise equal to `lo` and `hi`,
  respectively. The input values are unchanged, and the result does not
  alias either input C-language value.

For values with matching accumulator element types, the following identities
hold bitwise:

* `__riscv_ime_vunpairlo(__riscv_ime_vpair(lo, hi))` is equal to `lo`.

* `__riscv_ime_vunpairhi(__riscv_ime_vpair(lo, hi))` is equal to `hi`.

* `__riscv_ime_vpair(__riscv_ime_vunpairlo(src), __riscv_ime_vunpairhi(src))`
  is equal to `src`.

These are bitwise identities, not floating-point numerical comparisons;
they preserve every element bit, including signed-zero encodings, NaN
payloads, and non-ISO accumulator-format encodings.

The pair and unpair operations do not read or write memory, change `vl`,
`vtype`, `vstart`, or `fflags`, or perform element reordering, rounding,
saturation, sign extension, zero extension, or floating-point format
conversion.

[NOTE]
====
*Compiler-modeling guidance (non-normative).*

A compiler should model the pair and unpair pseudo-intrinsics as pure
register-group composition and extraction operations:

[cols="3,2,2,5", options="header"]
|===
| Operation | DEFs | USEs | Notes

| `__riscv_ime_vunpairlo` / `__riscv_ime_vunpairhi`
| result
| `src`
| Pure extraction of one half. There is no DEF of `src` and no hidden
  C-language alias between the result and `src`.

| `__riscv_ime_vpair`
| result
| `lo`, `hi`
| Pure composition. There is no DEF of either input and no hidden
  C-language alias between the result and either input.
|===

The backend may coalesce an unpair result with the corresponding physical
half of its source, or coalesce pair inputs with the corresponding halves of
the result. Zero-instruction lowering is an optimization, not a semantic
guarantee, and is valid only when register allocation, alignment, and
liveness permit it.

If an `m16` source and an extracted `m8` result must remain simultaneously
live, the backend preserves both values when either is subsequently
redefined, using distinct allocation or register moves as needed. Likewise,
`__riscv_ime_vpair` may require moves to place `lo` and `hi` into adjacent
halves of a sixteen-register-aligned group while preserving any still-live
input values.

The common load-pair-compute-unpair-store sequence can normally be allocated
so that neither pairing nor unpairing requires data movement.
====
...
returns the resulting value without modifying `vtype`, `vl`, or any
vector register.

When `vtype.vill` = 0 and the current (`VLEN`, `SEW`) configuration is
in the IME-legal domain, the returned value is a supported nonzero
`LAMBDA`.

A return value of zero means that no nonzero `LAMBDA` is currently
selected. An IME instruction that requires a dynamic `LAMBDA` raises an
illegal-instruction exception while no nonzero value is selected. A zero
result can occur in an unconfigured state, when `vtype.vill` = 1, or when
the current (`VLEN`, `SEW`) configuration is outside the IME-legal domain.
The intrinsic does not distinguish among these cases.

After establishing a valid vector configuration, software that needs to
establish a selected `LAMBDA` may call `__riscv_vsetlambda(0)` and inspect
its return value. Within the IME-legal domain, that operation preserves a
valid current value or selects a supported nonzero value. Outside that
domain, it returns zero. The operation does not repair `vtype.vill` = 1;
software must first establish a valid `vtype`.
...
Software that needs to enumerate all supported `LAMBDA` values may call
`__riscv_vsetlambda` with each valid nonzero candidate in
{`1`, `2`, `4`, `8`, `16`, `32`, `64`} and inspect the returned value.
Because those requests may change the selected `LAMBDA`, software should
restore its desired value after enumeration.
...
write request and returning the selected value established by that request.
It preserves all other `vtype` fields and preserves `vl`.

The `requested_lambda` argument denotes a semantic `LAMBDA` value, not the
raw three-bit `vtype.lambda[2:0]` encoding. Its valid domain is
{`0`, `1`, `2`, `4`, `8`, `16`, `32`, `64`}. Every possible three-bit
architectural `lambda[2:0]` bit pattern is defined; an out-of-domain value
can arise only at the C intrinsic interface.

A compiler shall diagnose an out-of-domain argument when its value is known
at compile time. If an out-of-domain value is supplied at runtime, the C
program has undefined behavior; this intrinsic specification provides no
guarantee about the request submitted, the returned value, or the resulting
architectural state.

An out-of-domain argument is distinct from a valid nonzero request whose
encoded value is unsupported for the current (`VLEN`, `SEW`) configuration.
A valid but unsupported request follows the WARL canonicalization rules
below.

If `requested_lambda` is a nonzero member of
{`1`, `2`, `4`, `8`, `16`, `32`, `64`}, the intrinsic requests the
corresponding nonzero `LAMBDA` value.
...
nonempty, so the established value is a supported nonzero value. For a
...
is nonempty within the IME-legal domain.
...
| `__riscv_vsetlambda` with a compile-time valid nonzero argument
| `lambda`
| `SEW`
| Encodable nonzero `LAMBDA` request followed by WARL canonicalization.
  A compile-time argument outside the intrinsic domain shall be diagnosed.

| `__riscv_vsetlambda` with a runtime argument
| `lambda` (may-DEF)
| `lambda`, `SEW`
| The caller shall supply a value in the intrinsic domain. The compiler
  cannot in general determine whether a valid runtime argument is zero.
  Conservative ordering should treat the operation as a lambda definition.
  A runtime value outside the intrinsic domain has undefined behavior.
|===
...
[#integrated-matrix-sew-lambda-selection]
===== Selecting lambda for an explicit SEW

The following non-instruction intrinsics select a supported geometry for
software dispatch without depending on the incoming vector configuration:

[source,c]
----
size_t __riscv_vsetlambda_e8 (size_t requested_lambda);
size_t __riscv_vsetlambda_e16(size_t requested_lambda);
size_t __riscv_vsetlambda_e32(size_t requested_lambda);
size_t __riscv_vsetlambda_e64(size_t requested_lambda);
----

In `__riscv_vsetlambda_e{S}`, the suffix supplies the architectural `SEW=S`
for which support is evaluated. For a computational intrinsic this is the
accumulator width, not a narrower packed input element width. No input
`LMUL` qualifier is needed: the supported nonzero `LAMBDA` set is a function
of (`VLEN`, `SEW`) and is independent of `LMUL`.

`requested_lambda` is a semantic value in
{`0`, `1`, `2`, `4`, `8`, `16`, `32`, `64`}, not a raw field encoding. A
compiler shall diagnose an out-of-domain argument when its value is known
at compile time. An out-of-domain runtime argument has undefined behavior,
as for the retained unsuffixed form.

Let `P_S` be the implementation-supported set of nonzero `LAMBDA` values
for the execution-time (`VLEN`, `SEW=S`) configuration. Selection is defined
as follows:

* If `P_S` is empty, return zero.
* If `requested_lambda` is zero and `P_S` is nonempty, return the largest
  member of `P_S`, independently of any previously selected `LAMBDA`.
* If a nonzero `requested_lambda` is a member of `P_S`, return that value.
* Otherwise, return the largest member of `P_S` no greater than the nonzero
  request; if no such member exists, return the smallest member of `P_S`.

Thus, zero requests have a _largest-supported-value_ meaning for these
SEW-qualified forms, not the preserve-or-initialize meaning of
`__riscv_vsetlambda(0)`. The nonzero-request algorithm is the same selection
algorithm as the architectural WARL rule, but evaluated for the explicit
`SEW` rather than an ambient `vtype` setting. A returned zero denotes the
absence of a supported nonzero geometry, not a geometry that can be used
for a matrix computation. The selector is not a general probe for the
presence of the Vector or IME instruction sets, and it does not establish
support for a particular arithmetic format or instruction variant.

These intrinsics return scalar selections. They do not establish a
persistent `SEW`, `LAMBDA`, `vl`, `LMUL`, or tail/mask policy on which
subsequent intrinsic calls may rely, and they do not promise to preserve
those hardware configuration fields. They do not modify live vector
values, application memory, or the floating-point environment. Each
computational intrinsic independently determines its required geometry as
described in <<integrated-matrix-intrinsic-compute-geometry>>.

For fixed execution-time `VLEN` and supported sets, identical suffixes and
requests shall return identical selections regardless of the intervening
vector operations. An implementation may use a temporary configuration and
readback to discover support, or compute the answer from known target
information. Such probing and its configuration effects are implementation
details; they shall not alter the values or required behavior of surrounding
intrinsic operations. In particular, a zero request cannot be implemented
by relying only on a preserve-or-initialize write of zero to the existing
hardware field, because that could retain a smaller supported value.

The unsuffixed `__riscv_vsetlambda` and `__riscv_ime_lambda` retain their
existing current-state-dependent contracts. They are not aliases of the
SEW-qualified selectors. Software shall not infer their input state or
query results from a preceding SEW-qualified selector call. In particular,
a preceding ordinary RVV `vsetvl` or `vsetvlmax` intrinsic is not a promise
that the physical `SEW` equals the width in its name.

[NOTE]
====
*Compiler-modeling guidance (non-normative).*

Model a SEW-qualified selector as producing a scalar result from its
argument, explicit SEW, execution-time VLEN, and implementation-supported
set. It has no incoming dependence on a programmer-maintained selected
lambda and does not define a persistent lambda mode for later computational
intrinsics. When target information proves the answer, the compiler may
fold the result; otherwise, it must obtain the execution-time answer.
Repeated calls may be combined when their relevant implementation
properties are unchanged, and a valid call with an unused result need not
emit a probing sequence. Any configuration changes used for probing are
handled internally by the backend, consistently with surrounding operations.

For a multiply-accumulate intrinsic, record the geometry requirement implied
by the accumulator SEW, `EMUL_C`, and execution-time VLEN, along with the
input `LMUL`, formats, vector length, and any microscaling qualifiers. The
backend establishes that operation's required configuration independently
of earlier selector calls. Ordinary value, memory, and floating-point
semantics continue to constrain optimization and scheduling.
====

===== Tile load and store (Zvvmtls) and transposing tile load and store (Zvvmttls)

The load intrinsics accept a base pointer, a leading-dimension `ld` in
storage elements, and a final vector-length `vl`, returning the loaded vector
type. The store intrinsics also accept the source vector and return `void`.
Forms without an `_L{N}` qualifier require an explicit `size_t lambda`
argument immediately before the final `size_t vl` argument. The `_L{N}`
forms specify the exact nonzero `LAMBDA` in the intrinsic name and do not
take a `lambda` argument. Neither family relies on a `LAMBDA` left by an
earlier selector or vector operation.

For tile loads and stores, the type-suffix encodes `LMUL`, the register-group
multiplier of the tile operand, not an accumulator `EMUL_C`. The storage
`SEW` is determined by the natural storage width of the logical type or by
an explicit `_as_e{S}` qualifier, as described below. These properties do
not determine a unique `LAMBDA` for a generic tile transfer.

[#integrated-matrix-runtime-lambda-load-store,reftext="Runtime lambda for tile load/store intrinsics"]
*Runtime lambda for tile load/store intrinsics*

The `lambda` argument specifies the exact semantic `LAMBDA` for that call,
not a raw three-bit field encoding or a preferred geometry. Its domain is
{`1`, `2`, `4`, `8`, `16`, `32`, `64`}. Zero is not permitted: it does not
mean preserve a previous selection, initialize a selection, or select the
largest supported value. This differs from the zero-request conventions of
the lambda-selection intrinsics.

The argument may be a compile-time constant or a runtime scalar value.
The program shall supply an in-domain value that is architecturally
permissible and implementation-supported for the execution-time (`VLEN`,
storage `SEW`) configuration. All other legality requirements in
<<integrated-matrix-tile-ls-legality>> also apply. In particular, the vector
length used by the transfer shall be a multiple of `lambda * LMUL`.
For an `_as_e{S}` form, support and legality are evaluated with `SEW=S`,
not with the narrower logical element width.

These are caller preconditions, as is the supported-geometry requirement
for computational intrinsics. The runtime-lambda forms are not support
queries and do not define a clamping, fallback, or checked-error interface.
A compiler shall diagnose an out-of-domain argument when its value is known
at compile time and may diagnose a geometry proved unsupported for the
compilation target. If a runtime call violates these domain or support
preconditions, the C interface does not guarantee a diagnostic or a
particular trap. This does not change the ISA legality rules or the
existing exception contract of the `_L{N}` forms.

For calls satisfying these preconditions, the compiler shall arrange for
the transfer to use the exact `lambda` argument. A different value shall
not be substituted because a configuration write canonicalizes a request.
The address calculation and register ordering use

    linesize = lambda * LMUL

in the corresponding instruction definition. The `ld=0` natural-dimension
convention is evaluated using this same exact value; it does not infer
`lambda`. Masking, storage-element units, and the distinction between
order-preserving and transposing transfers remain unchanged.

The explicit argument is required by every runtime-lambda tile load/store
form, including masked forms, storage-width-qualified forms, and applicable
overloaded short forms. It remains immediately before `vl`; a mask remains
the first argument, and a store's source vector precedes `lambda`. No
implicit-state overload omitting `lambda` is defined for these runtime forms.
Overload resolution does not derive `lambda` from the vector type or from a
subsequent use of the loaded value.

A SEW-qualified selector may supply the scalar value, but its returned
value must be passed explicitly to the transfer. Software whose memory
layout requires an exact requested geometry must verify that the selector
returned that value before using the existing data. No selector call is
required when the program has already established the exact supported value.
The load/store call does not establish or promise to preserve a hardware
configuration on which subsequent intrinsic calls may rely.

[NOTE]
====
*Compiler-modeling guidance (non-normative).*

Keep `lambda` as an explicit operand of the transfer in the intermediate
representation: it affects both the addresses accessed and the ordering of
the transferred elements. Record it together with the base pointer, `ld`,
storage `SEW`, `LMUL`, vector length, optional mask, and store data.
It is not merely a configuration hint.

The backend may use a nonzero instruction-lambda immediate when the value
is known, or establish the exact value in `vtype.lambda` at the required
storage `SEW` before issuing a dynamic-lambda instruction. It may reuse a
suitable configuration when doing so preserves the operation's semantics.
No incoming programmer-maintained lambda state is required. The compiler
need not emit a support probe before every operation when the program
satisfies the stated preconditions. These lowering choices add no ISA
operand and do not alter architectural WARL or exception behavior.
====

The following declarations show representative runtime-lambda `m1`
prototypes; replace `m1` with `m2`, `m4`, or `m8` as needed, and `i` with
`u` for unsigned integer variants where such variants are defined.

[source,c]
--
/* vmtl.v — order-preserving tile load */
vint8m1_t    __riscv_vmtl_v_i8m1  (const int8_t   *base, size_t ld, size_t lambda, size_t vl);
vint16m1_t   __riscv_vmtl_v_i16m1 (const int16_t  *base, size_t ld, size_t lambda, size_t vl);
vint32m1_t   __riscv_vmtl_v_i32m1 (const int32_t  *base, size_t ld, size_t lambda, size_t vl);
vint64m1_t   __riscv_vmtl_v_i64m1 (const int64_t  *base, size_t ld, size_t lambda, size_t vl);
vfloat16m1_t __riscv_vmtl_v_f16m1 (const _Float16 *base, size_t ld, size_t lambda, size_t vl);
vfloat32m1_t __riscv_vmtl_v_f32m1 (const float    *base, size_t ld, size_t lambda, size_t vl);
vfloat64m1_t __riscv_vmtl_v_f64m1 (const double   *base, size_t ld, size_t lambda, size_t vl);

/* vmts.v — order-preserving tile store */
void __riscv_vmts_v_i8m1  (int8_t   *base, size_t ld, vint8m1_t    vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_i16m1 (int16_t  *base, size_t ld, vint16m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_i32m1 (int32_t  *base, size_t ld, vint32m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_i64m1 (int64_t  *base, size_t ld, vint64m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_f16m1 (_Float16 *base, size_t ld, vfloat16m1_t vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_f32m1 (float    *base, size_t ld, vfloat32m1_t vs3, size_t lambda, size_t vl);
void __riscv_vmts_v_f64m1 (double   *base, size_t ld, vfloat64m1_t vs3, size_t lambda, size_t vl);

/* vmttl.v — transposing tile load */
vint8m1_t    __riscv_vmttl_v_i8m1  (const int8_t   *base, size_t ld, size_t lambda, size_t vl);
vint16m1_t   __riscv_vmttl_v_i16m1 (const int16_t  *base, size_t ld, size_t lambda, size_t vl);
vint32m1_t   __riscv_vmttl_v_i32m1 (const int32_t  *base, size_t ld, size_t lambda, size_t vl);
vint64m1_t   __riscv_vmttl_v_i64m1 (const int64_t  *base, size_t ld, size_t lambda, size_t vl);
vfloat16m1_t __riscv_vmttl_v_f16m1 (const _Float16 *base, size_t ld, size_t lambda, size_t vl);
vfloat32m1_t __riscv_vmttl_v_f32m1 (const float    *base, size_t ld, size_t lambda, size_t vl);
vfloat64m1_t __riscv_vmttl_v_f64m1 (const double   *base, size_t ld, size_t lambda, size_t vl);

/* vmtts.v — transposing tile store */
void __riscv_vmtts_v_i8m1  (int8_t   *base, size_t ld, vint8m1_t    vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_i16m1 (int16_t  *base, size_t ld, vint16m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_i32m1 (int32_t  *base, size_t ld, vint32m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_i64m1 (int64_t  *base, size_t ld, vint64m1_t   vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_f16m1 (_Float16 *base, size_t ld, vfloat16m1_t vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_f32m1 (float    *base, size_t ld, vfloat32m1_t vs3, size_t lambda, size_t vl);
void __riscv_vmtts_v_f64m1 (double   *base, size_t ld, vfloat64m1_t vs3, size_t lambda, size_t vl);
--

[#integrated-matrix-storage-width-load-store-intrinsics,reftext="Alternate-format and storage-width-qualified tile load/store intrinsics"]
...
vfloat8e4m3m1_t __riscv_vmtl_v_f8e4m3m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

vfloat8e5m2m1_t __riscv_vmtl_v_f8e5m2m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_f8e4m3m1(uint8_t *base, size_t ld, vfloat8e4m3m1_t vs3, size_t lambda, size_t vl);

void __riscv_vmts_v_f8e5m2m1(uint8_t *base, size_t ld, vfloat8e5m2m1_t vs3, size_t lambda, size_t vl);


/* BF16 natural-storage order-preserving load/store: SEW=16. */
vbfloat16m1_t __riscv_vmtl_v_bf16m1(const __bf16 *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_bf16m1(__bf16 *base, size_t ld, vbfloat16m1_t vs3, size_t lambda, size_t vl);


/* OFP4 and Int4 natural-storage order-preserving forms use SEW=8 storage. */
vfloat4e2m1m1_t __riscv_vmtl_v_f4e2m1m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

vint4m1_t __riscv_vmtl_v_i4m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

vuint4m1_t __riscv_vmtl_v_u4m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_f4e2m1m1(uint8_t *base, size_t ld, vfloat4e2m1m1_t vs3, size_t lambda, size_t vl);

void __riscv_vmts_v_i4m1(uint8_t *base, size_t ld, vint4m1_t vs3, size_t lambda, size_t vl);

void __riscv_vmts_v_u4m1(uint8_t *base, size_t ld, vuint4m1_t vs3, size_t lambda, size_t vl);
----
...
storage elements of width `S`, not in logical narrow elements. The runtime
`lambda` argument (or the `_L{N}` qualifier) supplies the exact `LAMBDA`
used with that storage `SEW`; `lambda * LMUL` is a number of storage
elements, not logical narrow elements. No numeric conversion is performed.
The operation is a bit-preserving load or store of packed storage elements.
...
vfloat16m1_t __riscv_vmtl_v_f16m1_as_e32(const uint32_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_f16m1_as_e32(uint32_t *base, size_t ld, vfloat16m1_t vs3, size_t lambda, size_t vl);


/* OFP8 logical elements packed in 32-bit storage positions. */
vfloat8e4m3m1_t __riscv_vmtl_v_f8e4m3m1_as_e32(const uint32_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_f8e4m3m1_as_e32(uint32_t *base, size_t ld, vfloat8e4m3m1_t vs3, size_t lambda, size_t vl);


/* OFP4 logical elements packed in 32-bit storage positions. */
vfloat4e2m1m1_t __riscv_vmtl_v_f4e2m1m1_as_e32(const uint32_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_f4e2m1m1_as_e32(uint32_t *base, size_t ld, vfloat4e2m1m1_t vs3, size_t lambda, size_t vl);


/* Int8 logical elements packed in 32-bit storage positions. */
vint8m1_t __riscv_vmtl_v_i8m1_as_e32(const uint32_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_i8m1_as_e32(uint32_t *base, size_t ld, vint8m1_t vs3, size_t lambda, size_t vl);


/* Int4 logical elements packed in 32-bit storage positions. */
vint4m1_t __riscv_vmtl_v_i4m1_as_e32(const uint32_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmts_v_i4m1_as_e32(uint32_t *base, size_t ld, vint4m1_t vs3, size_t lambda, size_t vl);
----
...
vfloat8e4m3m1_t __riscv_vmttl_v_f8e4m3m1(const uint8_t *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmtts_v_f8e4m3m1(uint8_t *base, size_t ld, vfloat8e4m3m1_t vs3, size_t lambda, size_t vl);


/* BF16 natural-storage transposing load/store: SEW=16. */
vbfloat16m1_t __riscv_vmttl_v_bf16m1(const __bf16 *base, size_t ld, size_t lambda, size_t vl);

void __riscv_vmtts_v_bf16m1(__bf16 *base, size_t ld, vbfloat16m1_t vs3, size_t lambda, size_t vl);
----
...
[#integrated-matrix-immediate-lambda-mask-qualifiers,reftext="Immediate-lambda and mask qualifiers"]
*Immediate-lambda and mask qualifiers*

When an immediate `LAMBDA` override is required, an `_L{N}` variant
encodes the requested value, where `N` ∈ {`1`, `2`, `4`, `8`, `16`, `32`,
`64`}, directly in the intrinsic name. The compiler can statically verify
that `N` is an encodable nonzero `LAMBDA` value and can place its
corresponding three-bit encoding in the instruction-immediate field. These
forms retain their existing signatures and do not accept a `lambda`
argument. The `_L{N}` value is exact and does not depend on the incoming
`vtype.lambda` value.

Encodability, architectural permissibility, and implementation support are
distinct. An encodable `N` may be architecturally impermissible for the
execution-time (`VLEN`, `SEW`) geometry, and an architecturally permissible
`N` may still be unsupported by a particular implementation. The immediate
requests the exact value `N` and is not WARL-canonicalized. If `N` is not
supported for the execution-time (`VLEN`, `SEW`) configuration, the tile
load/store instruction raises an illegal-instruction exception as specified
in <<integrated-matrix-tile-ls-legality>>.

A compiler may diagnose an `_L{N}` intrinsic when the selected compilation
target proves that `N` is unsupported. Such a diagnostic is not required
when target support cannot be determined statically.
...
The mask type follows the standard RISC-V Vector Intrinsics mask-ratio
rule. For a tile load/store whose storage element width (instruction
`SEW`) is `S` and whose tile operand uses `LMUL`, the mask-ratio parameter
is

    N = S / LMUL

and the mask argument type is `vbool{N}_t`. Here `N` denotes the mask
ratio, not the width of a mask element; every mask element is one bit.
For an unqualified natural-storage intrinsic, `S` is the natural storage
width of the logical type. In particular, unqualified OFP4 and Int4
intrinsics use `S=8`. For an `_as_e{S}` intrinsic, `S` is the storage
width named by that qualifier.

Each mask bit controls one `S`-bit storage element. If multiple logical
narrow elements are packed within one storage element, the mask bit
controls the complete storage element and therefore all logical elements
packed within it.

[source,c]
--
/* vmtl.v masked: */
vint8m1_t  __riscv_vmtl_v_i8m1_m  (vbool8_t mask,  const int8_t  *base, size_t ld, size_t lambda, size_t vl);
vint32m1_t __riscv_vmtl_v_i32m1_m (vbool32_t mask, const int32_t *base, size_t ld, size_t lambda, size_t vl);

/* SEW=32, LMUL=2: mask ratio is 32/2 = 16. */
vint32m2_t __riscv_vmtl_v_i32m2_m (vbool16_t mask, const int32_t *base, size_t ld, size_t lambda, size_t vl);
/* vmts.v masked: */
void __riscv_vmts_v_i8m1_m  (vbool8_t mask,  int8_t  *base, size_t ld, vint8m1_t  vs3,
    size_t lambda, size_t vl);
void __riscv_vmts_v_i32m1_m (vbool32_t mask, int32_t *base, size_t ld, vint32m1_t vs3,
    size_t lambda, size_t vl);
/* vmttl.v masked: */
vint32m1_t __riscv_vmttl_v_i32m1_m (vbool32_t mask, const int32_t *base, size_t ld, size_t lambda, size_t vl);
/* vmtts.v masked: */
void __riscv_vmtts_v_i32m1_m (vbool32_t mask, int32_t *base, size_t ld, vint32m1_t vs3,
    size_t lambda, size_t vl);
--
...
For example, an FP16 logical tile loaded from 32-bit storage uses the
mask ratio derived from `S=32`, rather than from the 16-bit logical
element width:

[source,c]
----
/* FP16 logical elements in SEW=32 storage, LMUL=1:
   mask ratio is 32/1 = 32. */
vfloat16m1_t __riscv_vmtl_v_f16m1_as_e32_m(vbool32_t mask, const uint32_t *base, size_t ld,
    size_t lambda, size_t vl);

/* FP16 logical elements in SEW=32 storage, LMUL=2:
   mask ratio is 32/2 = 16. */
vfloat16m2_t __riscv_vmtl_v_f16m2_as_e32_m(vbool16_t mask, const uint32_t *base, size_t ld,
    size_t lambda, size_t vl);

/* An immediate LAMBDA qualifier does not change the mask ratio. */
...
widening multiply-accumulate uses `SEW=32` for both transfers and the
computation. Here, `lambda` is an explicit scalar holding the exact
supported value used for the panel layout, not an incoming hardware
setting. This code path is selected only when that value gives
`EMUL_C=16` at `SEW=32`, matching the type of `c`:

    VLEN / (32 * lambda * lambda) = 16.

[source,c]
----
/* Each call specifies its own geometry; A/B transfers use LMUL=1. */
/* Each 32-bit storage element contains two packed FP16 values. */

vfloat16m1_t a = __riscv_vmtl_v_f16m1_as_e32(A_packed, lda_in_e32, lambda, vl_in_e32);

vfloat16m1_t b = __riscv_vmtl_v_f16m1_as_e32(B_packed, ldb_in_e32, lambda, vl_in_e32);
...
The accumulator register group C uses EMUL_C = VLEN ÷ (SEW × LAMBDA²) registers.
...
LMUL directly.
The tables below list representative prototypes for VLEN=256 and LAMBDA=2 (giving
...
[source,c]
--
/* SEW=16 accum / EEW=8 inputs,  VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
vint16m4_t __riscv_vwmmacc_vv_i16m4   (vint16m4_t vd, vint8m1_t  vs1, vint8m1_t  vs2, size_t vl);
/* SEW=32 accum / EEW=16 inputs, VLEN=256, LAMBDA=2: EMUL_C=2.  LMUL=1 (default): */
vint32m2_t __riscv_vwmmacc_vv_i32m2   (vint32m2_t vd, vint16m1_t vs1, vint16m1_t vs2, size_t vl);
/* SEW=64 accum / EEW=32 inputs, VLEN=256, LAMBDA=2: EMUL_C=1.  LMUL=1 (default): */
...
[source,c]
--
/* SEW=32 accum / EEW=8 inputs,  VLEN=256, LAMBDA=2: EMUL_C=2.  LMUL=1 (default): */
vint32m2_t __riscv_vqmmacc_vv_i32m2  (vint32m2_t vd, vint8m1_t  vs1, vint8m1_t  vs2, size_t vl);
/* SEW=64 accum / EEW=16 inputs, VLEN=256, LAMBDA=2: EMUL_C=1.  LMUL=1 (default): */
...
[source,c]
--
/* Zvvi4i8mm: Int4→Int8, SEW=8, VLEN=256, LAMBDA=2: EMUL_C=8.  LMUL=1 (default): */
...
[source,c]
--
/* Zvvi4i16mm: Int4→Int16, SEW=16, VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
...
[source,c]
--
/* vwmmacc.vv signed A × unsigned B; SEW=32, VLEN=256, LAMBDA=2: EMUL_C=2, LMUL=1 */
vint32m2_t __riscv_vwmmacc_vv_i32m2_su(vint32m2_t vd, vint16m1_t  vs1,
                                        vuint16m1_t vs2, size_t vl);
/* vwmmacc.vv unsigned A × signed B; SEW=32, VLEN=256, LAMBDA=2: EMUL_C=2, LMUL=1 */
...
[source,c]
--
/* OFP8 E4M3 accum / OFP4 E2M1 inputs, VLEN=256, LAMBDA=2: EMUL_C=8.  LMUL=1 (default): */
...
/* FP16 accum / OFP8 E4M3 inputs, VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
vfloat16m4_t __riscv_vfwmmacc_vv_f16m4_f8e4m3m1(vfloat16m4_t vd,
                                                  vfloat8e4m3m1_t vs1, vfloat8e4m3m1_t vs2, size_t vl);
/* BF16 accum / OFP8 E4M3 inputs, VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
vbfloat16m4_t __riscv_vfwmmacc_vv_bf16m4_f8e4m3m1(vbfloat16m4_t vd,
                                                    vfloat8e4m3m1_t vs1, vfloat8e4m3m1_t vs2, size_t vl);
/* FP32 accum / FP16 inputs, VLEN=256, LAMBDA=2: EMUL_C=2.  LMUL=1 (default): */
vfloat32m2_t __riscv_vfwmmacc_vv_f32m2(vfloat32m2_t vd,
                                        vfloat16m1_t vs1, vfloat16m1_t vs2, size_t vl);
/* FP64 accum / FP32 inputs, VLEN=256, LAMBDA=2: EMUL_C=1.  LMUL=1 (default): */
...
[source,c]
--
/* FP16 accum / OFP4 E2M1 inputs, VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
vfloat16m4_t __riscv_vfqmmacc_vv_f16m4_f4e2m1m1(vfloat16m4_t vd,
                                                   vfloat4e2m1m1_t vs1, vfloat4e2m1m1_t vs2, size_t vl);
/* BF16 accum / OFP4 E2M1 inputs, VLEN=256, LAMBDA=2: EMUL_C=4.  LMUL=1 (default): */
vbfloat16m4_t __riscv_vfqmmacc_vv_bf16m4_f4e2m1m1(vbfloat16m4_t vd,
                                                     vfloat4e2m1m1_t vs1, vfloat4e2m1m1_t vs2, size_t vl);
/* FP32 accum / OFP8 E4M3 inputs, VLEN=256, LAMBDA=2: EMUL_C=2.  LMUL=1 (default): */
vfloat32m2_t __riscv_vfqmmacc_vv_f32m2_f8e4m3m1(vfloat32m2_t vd,
                                                   vfloat8e4m3m1_t vs1, vfloat8e4m3m1_t vs2, size_t vl);
/* FP64 accum / BF16 inputs, VLEN=256, LAMBDA=2: EMUL_C=1.  LMUL=1 (default): */
vfloat64m1_t __riscv_vfqmmacc_vv_f64m1_bf16m1(vfloat64m1_t vd,
                                                 vbfloat16m1_t vs1, vbfloat16m1_t vs2, size_t vl);
/* FP64 accum / FP16 inputs, VLEN=256, LAMBDA=2: EMUL_C=1.  LMUL=1 (default): */
...
Because `EMUL_C = VLEN / (SEW × LAMBDA²)`, the accumulator register-group
multiplier varies across implementations with different `VLEN` and `LAMBDA`
values, even for a fixed `SEW` tied to an output type.
The type suffix in a long-form multiply-accumulate intrinsic name directly
encodes `EMUL_C`, whereas an overloaded name leaves `EMUL_C` implicit.
Either way, source code with IME intrinsics is tied to a specific
combination of input and output types and an `EMUL_C` value.
...
Although more general assembly code is possible, industry practice commonly
favors compiler intrinsics.
The recommended approach for writing portable code with IME intrinsics is to
package multiple code paths in the same executable, each optimized for a
specific `EMUL_C` value.
The appropriate code path may be selected at run time using a SEW-qualified
lambda selector and the computed value of
`EMUL_C = VLEN / (SEW × LAMBDA²)`. The selector supplies a scalar value for
dispatch, not a persistent hardware mode. Each computational intrinsic
executes with the geometry implied by its types and the execution-time
VLEN. See <<microdgemm-intrinsics-example>> for a complete five-way example.
...
The semantics of each instruction are expressed in a SAIL-like syntax, using the following
...
// Return true when two architectural vector-register groups share at least
// one register. Each group denotes the half-open range [base, base + regs).
function ime_reg_groups_overlap(base_a : regidx, regs_a : int,
                                base_b : regidx, regs_b : int) -> bool = {
  let a0 : int = unsigned(base_a);
  let b0 : int = unsigned(base_b);

  not (a0 + regs_a <= b0 | b0 + regs_b <= a0)
}

// Check register-group alignment, bounds, and destination/source overlap for
// a multiply-accumulate. The A and B input operands (vs1, vs2) form
// LMUL-register groups; the C accumulator (vd) forms an EMUL_C-register
// group. The two input groups may overlap each other, but the destination
// group shall not overlap either input group.
function check_gemm_reg_groups(g : gemm_geom,
  ...
  let vd_overlaps_input : bool =
    ime_reg_groups_overlap(vd, g.EMUL_C, vs1, g.LMUL) |
    ime_reg_groups_overlap(vd, g.EMUL_C, vs2, g.LMUL);

  if vd_overlaps_input then
    return Illegal_Instruction()
}
...
// Map sequential tile element index i to the flat register-group element
// index. row_elems_per_reg is the number of consecutive elements from
// each tile row stored in one register, and group_regs is the number of
...
// Each register contributes lambda * W consecutive logical K elements
// from each row.
function mat_A_idx(i : int, k : int, K_eff : int,
                   LMUL : int, lambda : int, W : int,
                   epr : int) -> int =
...
// N x K_eff view of the same register contents. Each register contributes
// lambda * W consecutive logical K elements from each row.
...
//   E8M0 → 8. This version defines no other scale width.
...
//   FP format, rounded per rm. The scale format is determined by the input
//   data type. This helper updates fflags for exceptions raised by
//   conversion to the target FP format.
...
// other bulk-normalized fixed-point schemes, and Kulisch-style
// accumulators.
//
...
// normalization -- such as RVBNA, the flagship psm=1 algorithm -- which
// aligns and sums all g_len x W terms together with a single terminal
// normalization. fmt_A and fmt_B give the input formats, and fmt_C gives the C
...
//
// The list presents the products in increasing logical K order. The
// disclosed reduction may be order-sensitive and may reassociate, pair, or
// reorder the products internally, but its SAIL definition must completely
// specify that behavior. The returned value has not yet been rounded to the
// C accumulator format; any terminal rounding (e.g. RVBNA's round-to-odd) is
// applied by the separate rnd step.
//
...
// both be conforming, provided each discloses its algorithm, ordering
// behavior, parameters, and exception-flag behavior.
val fp_disclosed_reduce_group : (list(fp_internal), fp_fmt, fp_fmt, fp_fmt, int, int) -> fp_internal

// The product-gathering loop below prepends each newly computed product.
// Reverse that temporary list once so fp_disclosed_reduce_group receives the
// products in increasing logical K order.
val fp_reverse_products_acc :
  (list(fp_internal), list(fp_internal)) -> list(fp_internal)
function fp_reverse_products_acc(xs, acc) =
  match xs {
    [||]      => acc,
    x :: rest => fp_reverse_products_acc(rest, x :: acc)
  }

val fp_reverse_products : list(fp_internal) -> list(fp_internal)
function fp_reverse_products(xs) =
  fp_reverse_products_acc(xs, [||])
  ...
  match psm {
    PSM_EXACT => {
      ...
      foreach (k from k_lo to k_hi) {
        let a_flat : int = mat_A_idx(i, k, g.K_eff, g.LMUL, g.lambda, g.W, g.epr_A);
        let b_flat : int = mat_B_idx(k, j, g.K_eff, g.LMUL, g.lambda, g.W, g.epr_A);
    ...
    PSM_DISCLOSED => {
      ...
      // bulk operation so that a disclosed bulk-normalization algorithm
      // (e.g. RVBNA) can normalize all terms together. The loop visits k in
      // increasing order and prepends each product, so prods_rev is reversed
      // once before invoking the disclosed reduction.
      var prods_rev : list(fp_internal) = [||];
      foreach (k from k_lo to k_hi) {
        let a_flat : int = mat_A_idx(i, k, g.K_eff, g.LMUL, g.lambda, g.W, g.epr_A);
        let b_flat : int = mat_B_idx(k, j, g.K_eff, g.LMUL, g.lambda, g.W, g.epr_A);
        ...
        let prod : fp_internal = fp_mul_exact(a_bits, fmt_A, b_bits, fmt_B);
        prods_rev = prod :: prods_rev
      };
      let prods : list(fp_internal) = fp_reverse_products(prods_rev);
...
// scale values into the C accumulator format using rm, forms their product
// with fp_mul, and returns (scale_A × scale_B, is_nan). This operation follows
...
function read_block_scales(vm : bit, i : int, j : int, s : int,
  ...
  let sA : bits(EEW_C) =
    if vm == 0
    then decode_scale(pair_A[sw-1..0], scale_fmt, EEW_C, fmt_C, rm)
    else fp_one(EEW_C, fmt_C);
  let sB : bits(EEW_C) =
    if vm == 0
    then decode_scale(pair_B[2*sw-1..sw], scale_fmt, EEW_C, fmt_C, rm)
    else fp_one(EEW_C, fmt_C);
...
// Raises Illegal_Instruction if SEW × LAMBDA < pw (pair width) or
...
Description::
Interpreting _vs1_ as A~tile~ and _vs2_ as
B~tile~^T^, the instruction computes
...
+
_vs1_ is interpreted as the M × K~eff~ tile A~tile~ in row-major register layout, and _vs2_ is interpreted as the K~eff~ × N tile B~tile~^T^ in column-major register layout,
...
+
K_effective = LAMBDA × 8 × LMUL.
`VL` selects how many columns of B~tile~^T^, and therefore how many columns of C, are active; VL must be a multiple of LAMBDA × LMUL.
...
Exceptions::
_Illegal Instruction_ is raised if any of the following conditions hold:
+
* `vstart` ≠ 0.
* SEW < 32 (EEW = SEW÷8 would be sub-nibble).
* VL is not a multiple of LAMBDA × LMUL.
* EMUL_C (= VLEN ÷ (SEW × LAMBDA²)) is not in \{1, 2, 4, 8, 16}.
* _vs1_ or _vs2_ is not aligned for LMUL.
* _vd_ is not aligned for EMUL_C.
* The _vd_ register group overlaps the _vs1_ or _vs2_ register group.
...
Description::
Interpreting _vs1_ as A~tile~ and _vs2_ as
B~tile~^T^, the instruction computes the exact integer
tile product
...
Mnemonic::
vmtl.v _vd_, (_rs1_), _rs2_ [, _L`LAMBDA`_] [, _vm_]
...
if _rs2_ holds zero (0), the leading dimension `LD` is set to LAMBDA × LMUL.
When _LLAMBDA_ is specified, it overrides `lambda[2:0]` from `vtype` for this instruction.
...
contiguous segments with a stride of `LD` elements between them.
For LMUL=1 this simplifies to LAMBDA-element segments.
...
Mnemonic::
vmts.v _vs3_, (_rs1_), _rs2_ [, _LLAMBDA_] [, _vm_]
...
Mnemonic::
vmttl.v _vd_, (_rs1_), _rs2_ [, _LLAMBDA_] [, _vm_]
...
_rs1_ holds the matrix base address; _rs2_ holds the leading dimension `LD` in elements of width SEW.
If _rs2_ holds zero (0), the leading dimension `LD` is set to VLEN / (SEW × LAMBDA),
which is the natural maximum width of a B tile and height of an A or C tile.
The parameter _LAMBDA_ refers to the result matrix tile of the transposing load operation.
When _LLAMBDA_ is specified, it overrides `lambda[2:0]` from `vtype` for this instruction.
Inactive elements (masked off) are left undisturbed in the destination register.
Tail elements (index ≥ VL) are left undisturbed in the destination register.
The tail-agnostic (`vtype.vta`) and mask-agnostic (`vtype.vma`) fill policies do not apply to tile loads.
...
Mnemonic::
vmtts.v _vs3_, (_rs1_), _rs2_ [, _LLAMBDA_] [, _vm_]
...
_rs1_ holds the matrix base address; _rs2_ holds the leading dimension `LD` in elements of width SEW.
If _rs2_ holds zero (0), the leading dimension `LD` is set to VLEN / (SEW × LAMBDA).
The parameter _`LAMBDA`_ refers to the source matrix tile of the transposing store operation.
When _LLAMBDA_ is specified, it overrides `lambda[2:0]` from `vtype` for this instruction.
Inactive elements (masked off) are not written to memory and do not raise exceptions.
Tail elements (index ≥ VL) are not written to memory and do not raise exceptions.
+
