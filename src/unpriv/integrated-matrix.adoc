...
The two legal choices for `LAMBDA` (2 and 4), and the corresponding values for `M` (16 and 8), are illustrated in <<ime-vec-reg-geometry-L32>>.
...
.Geometry of matrix tiles fitting in a single vector register for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 32.) There are two legal choices for the implementation parameter `LAMBDA`, and corresponding `M`. (a) `LAMBDA` = 2, `M` = 16. (b) `LAMBDA` = 4, `M` = 8.
...
.Geometry of the computation of C ← A × B^T^ + C for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN`/`SEW`) = 32.) There are two legal choices for the implementation parameter `LAMBDA`, which impacts the shapes of the A, B, and C tiles, including the multiplicity `EMUL_C` of the vector register group holding the accumulator tile C. (a) `LAMBDA` = 2, `M` = 16, `EMUL_C` = 8. (b) `LAMBDA` = 4, `M` = 8, `EMUL_C` = 2.
...
In general, if `L` = (`VLEN`/`SEW`) is a perfect square (an even power of 2), then the valid choices for `LAMBDA` are {`L`^½^, `L`^½^ ÷ 2, `L`^½^ ÷ 4}.
If `L` = (`VLEN`/`SEW`) is two times a perfect square (an odd power of 2), then the valid choices for `LAMBDA` are {(`L` ÷ 2)^½^, (`L` ÷ 2)^½^ ÷ 2}.
...
*Dependence on `L`, and hence on `SEW`.* As shown in <<ime-emulc-examples>> and <<ime-emulc-examples-L32>>, different choices of `LAMBDA` for a given `L` = `VLEN`/`SEW` lead to different shapes of the multiplier array. Since `VLEN` is fixed for an implementation, the flexibility in `LAMBDA` lets the same multiplier array shape be used for different `SEW`. For example, if `VLEN`=512, a 16 × 16 array can be used with both (`SEW` = 32, `LAMBDA` = 1) and (`SEW` = 16, `LAMBDA` = 2). In general, a `M` × `M` multiplier array can be used by setting `LAMBDA` = `L` ÷ `M` (when legal). Narrower C elements (smaller `SEW`) raise the value of `L` and therefore require a larger `LAMBDA` to result in the same multiplier shape.
