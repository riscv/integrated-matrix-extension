...
[#ime-block-scale-v0]
.Block scales are folded into `v0` in pairs, separated by the row stride R = `LAMBDA` × SEW / _pw_. Each block-scale pair is used for `bs` consecutive tile elements in the corresponding row of A~tile~ and column of B~tile~^T^. In matrix-tile representation, `v0` has the paired scales arranged in the correct scale rows. In a multi-lane setup, the scales are distributed across the lanes appropriately.
