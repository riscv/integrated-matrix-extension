...
`M` = `VLEN` ÷ (`SEW` × λ) and `K_eff` = λ × `W` × `LMUL`.
...
To define the shape of a tile contained in a single vector registers, we introduce an additional implementation-choice parameter λ that is the width of such tile in elements of `SEW`.
The height of the tile is then `M` = (`VLEN` ÷ `SEW`) ÷ λ.
The IM extension imposes two more restrictions on λ. (An additional third restriction will be introduced later.)
First, λ, and consequently `M`, must be a power of 2 ≥ 1.
Second, we must have `M` ≥ λ or, equivalently, λ ≤ (`VLEN` ÷ `SEW`)^½^.

As an example of the implication of the rules for λ, let us consider the case `VLEN` = 512 and `SEW` = 32.
In this case, (`VLEN` ÷ `SEW`) = 16 and, therefore there are only three possible choices for λ: 1, 2, or 4.
...
.Geometry of matrix tiles fitting in a single vector register for `VLEN` = 512 and `SEW` = 32. (In general, for (`VLEN` ÷ `SEW`) = 16.) There are three legal choices for the implementation parameter λ, and corresponding `M`. (a) λ = 1, `M` = 16. (b) λ = 2, `M` = 8. (c) λ = 4, `M` = 4.
...
Since the `M` × `K` shape of the input tiles A and B that fit in a single vector register depends on the parameter λ, so does the shape `M` × `M` of the accumulator tile C.
Correspondingly, the multiplicity of the vector register group holding tile C, `EMUL_C`, is also a function of λ, `EMUL_C` = `M` ÷ λ = (`VLEN` ÷ `SEW`) ÷ λ^2^.
The three possible configurations for `VLEN` = 512 and `SEW` = 32 are shown in <<ime-emulc-examples>>.

[#ime-emulc-examples]
.Geometry of the computation of C ← A × B^T^ + C for `VLEN` = 512 and `SEW` = 32. (In general, for (`VLEN` ÷ `SEW`) = 16.) There are three legal choices for the implementation parameter λ, which impacts the shapes of the A, B, and C tiles, including the multiplicity `EMUL_C` of the vector register group holding the accumulator tile C. (a) λ = 1, `M` = 16, `EMUL_C` = 16. (b) λ = 2, `M` = 8, `EMUL_C` = 4. (c) λ = 4, `M` = 4, `EMUL_C` = 1.
image::png/ime-emulc-examples.png[width=80%, align=center, alt="Diagram of C ← A × B^T^ + C computation for different choices of λ."]

We now introduce the third restriction on λ.
...
This requires a minimum values of λ when (`VLEN` ÷ `SEW`) is large.
As an example, let us now consider the case  `VLEN` = 1024 and `SEW` = 32, leading to (`VLEN` ÷ `SEW`) = 32.
The two legal choices for λ (2 and 4), and the corresponding values for `M` (16 and 8), are illustrated in <<ime-vec-reg-geometry-L32>>.
A choice of λ = 1 would result in `EMUL_C` = 32, leaving no space for the A and B tiles.
The two possible configurations for `VLEN` = 1024 and `SEW` = 32 are shown in <<ime-emulc-examples-L32>>.

[#ime-vec-reg-geometry-L32]
.Geometry of matrix tiles fitting in a single vector register for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN` ÷ `SEW`) = 32.) There are two legal choices for the implementation parameter λ, and corresponding `M`. (a) λ = 2, `M` = 16. (b) λ = 4, `M` = 8.
...
.Geometry of the computation of C ← A × B^T^ + C for `VLEN` = 1024 and `SEW` = 32. (In general, for (`VLEN` ÷ `SEW`) = 32.) There are two legal choices for the implementation parameter λ, which impacts the shapes of the A, B, and C tiles, including the multiplicity `EMUL_C` of the vector register group holding the accumulator tile C. (a) λ = 2, `M` = 16, `EMUL_C` = 8. (b) λ = 4, `M` = 8, `EMUL_C` = 2.
image::png/ime-emulc-examples-L32.png[width=80%, align=center, alt="Diagram of C ← A × B^T^ + C computation for different choices of λ."]

In general, if `L` = (`VLEN` ÷ `SEW`) is a perfect square (an even power of 2), then the valid choices for λ are {`L`^½^, `L`^½^ ÷ 2, `L`^½^ ÷ 4}.
If `L` = (`VLEN` ÷ `SEW`) is two times a perfect square (an odd power of 2), then the valid choices for λ are {(`L` ÷ 2)^½^, (`L` ÷ 2)^½^ ÷ 2}.
The legal choices of λ, and corresponding values of `EMUL_C`, for all possible values of (`VLEN` ÷ `SEW`) are shown in <<ime-emulc-table>>.

[#ime-emulc-table]
.Table of legal implementation choices of λ for different values of `L` = (`VLEN` ÷ `SEW`). The entries in the table show the resulting multiplicity `EMUL_C` of the vector register group holding tile C.
image::png/ime-emulc-table.png[width=80%, align=center, alt="Table of choices of λ as a function of (`VLEN` ÷ `SEW`)."]

We see in <<ime-emulc-table>> that the majority of cases support either 2 or 3 choices for λ.
...
but are otherwise portable across specific choices of `VLEN` and λ.
...
Arguments `L` and `lambda` are (`VLEN` ÷ `SEW`) and the selected λ, respectively.
...
.Geometry of the five microkernel variants. (a) `EMUL_C`=16, λ=(`VLEN` ÷ `SEW`)^½^ ÷ 4. (b) `EMUL_C`=8, λ=((`VLEN` ÷ `SEW`) ÷ 2)^½^ ÷ 2. (c) `EMUL_C`=4, λ=(`VLEN` ÷ `SEW`)^½^ ÷ 2. (d) `EMUL_C`=2, λ=((`VLEN` ÷ `SEW`) ÷ 2)^½^. (e) `EMUL_C`=1, λ=(`VLEN` ÷ `SEW`)^½^.
...
*Dependence on `L`, and hence on `SEW`.* As shown in <<ime-emulc-examples>> and <<ime-emulc-examples-L32>>, different choices of λ for a given `L` = `VLEN` ÷ `SEW` lead to different shapes of the multiplier array. Since `VLEN` is fixed for an implementation, the flexibility in λ lets the same multiplier array shape be used for different `SEW`. For example, if `VLEN`=512, a 16 × 16 array can be used with both (`SEW` = 32, λ = 1) and (`SEW` = 16, λ = 2). In general, a `M` × `M` multiplier array can be used by setting λ = `L` ÷ `M` (when legal). Narrower C elements (smaller `SEW`) raise the value of `L` and therefore require a larger λ to result in the same multiplier shape.
...
*Cacheline utilization of the tile loads.* The base λ can be extended along K by `LMUL`, giving an effective contraction depth `K_eff` = λ · `LMUL` (<<ISA-control-geometry>>, <<ime-tile-lmul-fig>>). Each matrix row streamed by an order-preserving tile load `vmtl` is therefore `K_eff` · `SEW` = λ · `LMUL` · `SEW` bits of contiguous data, and this length determines how well each row maps onto a memory cacheline. For `SEW` = 32 and a 512-bit (64-byte) cacheline: configuration (a), with λ = 1, streams at most 8 · 32 = 256 bits per row even at the maximum `LMUL` = 8, filling only half a cacheline; configuration (b), with λ = 2, reaches a full 512-bit line at `LMUL` = 8; and configuration (c), with λ = 4, reaches a full line already at `LMUL` = 4. A λ that is too small can thus leave `vmtl` unable to issue full-cacheline row transfers within the available `LMUL`, wasting bandwidth on partial lines. This gives a lower bound on the useful λ that is independent of, and may exceed, the minimum required for legality.

*Summary of the trade-off.* λ (and thus `EMUL_C`) is selected to balance three competing objectives: high compute intensity and peak throughput, which favour small λ (large C-panel, large `EMUL_C`); bounded multiplier area and power, which favour large λ; and full-cacheline tile loads, which impose a minimum useful λ. The resulting choice must in all cases satisfy the legality constraints of <<ime-emulc-table>>, in particular `EMUL_C` ∈ {1, 2, 4, 8, 16} and the reservation of register groups for the A and B operands.
...
The geometry of the multiplier and the tiles is defined by the new `lambda` (the selected λ) field, which is encoded in 3 bits in the `vtype` CSR, in combination with the per-instruction widening factor `W` and the standard vector-configuration values `LMUL`, `SEW`, and `VLEN`.
...
====
[.text-center]
C~i,j~ ← C~i,j~ + `SUM`~k=0~^λ×W−1^ A~tile~[i,k] × B~tile~^T^[k,j]
====
...
For unscaled floating-point multiply-accumulate instructions, every group within an `LMUL=1` step has exactly `G` sub-dot-products, and the number of groups in the computation of C~i,j~ above is λ ÷ `G`.
...
.Block scaling multiplication example with SEW=16, λ=4, W=4, LMUL=1. Block scales are applied along rows of A~tile~ and columns of B~tile~^T^.
...
[#ime-block-scale-v0]
.Block scales are folded into `v0` in pairs, separated by the row stride R = λ × SEW / _pw_. Each block-scale pair is used for `bs` consecutive tile elements in the corresponding row of A~tile~ and column of B~tile~^T^. In matrix-tile representation, `v0` has the paired scales arranged in the correct scale rows. In a multi-lane setup, the scales are distributed across the lanes appropriately.
...
`VL` selects how many columns of B~tile~^T^, and therefore how many columns of C, are active; VL must be a multiple of λ × LMUL.
