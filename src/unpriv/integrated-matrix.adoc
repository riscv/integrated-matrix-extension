...
widening factor, and `LAMBDA` is the IM tile-layout parameter introduced
below.
`EMUL_C` is the register-group multiplier for the output C and is computed as
`EMUL_C` = (`VLEN`/`SEW`) ÷ `LAMBDA`^2^.
A, B, and C are row-major matrix tiles.
A and B are the input tiles, both of shape `M` × `K_eff`. C, of shape `M` × `M`, is the accumulator (output) tile.
`M` = `VLEN` ÷ (`SEW` × `LAMBDA`) and `K_eff` = `LAMBDA` × `W` × `LMUL`.
...
<<tile-dimensions>> shows how the tile dimensions follow from the widening, `LAMBDA`, and LMUL settings.
...
|K~eff~     | `LAMBDA` × W × LMUL         | number of input elements along the inner (K) dimension
|M = N      | (VLEN ÷ SEW) ÷ `LAMBDA`     | rows = columns of the square accumulator tile
|===
