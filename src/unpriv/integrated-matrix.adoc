...
Arguments `A`  and `B` are the input matrices. The result C is stored in vector register v16-v31.
Arguments `M`, `N`, and `K` define the shape of the matrix multiply, following BLAS convention.
Arguments `L` and `lambda` are (`VLEN`/`SEW`) and the selected `LAMBDA`, respectively.
...
The computation of `EMUL_C`, and the consequent decision of which code version to execute,
are performed only once per microkernel execution.
Once the appropriate code path is selected, the microkernel code is tight, with
only input loads (`vle64` instructions) and computations (`vfmmacc` instructions) (and address updates) in the loop body.
...
{
    assert(0 == K % lambda);           // <1>
    u32 emul_c = L/(lambda * lambda);  // <2>

    vsetvli(5, 0, 64, 1, true, true);
    for (u32 r=16; r<32; r++) vxor.vv(r, r, r);         // <3>
...
----
<1> For simplicity, `K` is an integer multiple of `lambda`.
<2> `emul_c` is the number of vector registers occupied by one physical
    C accumulator tile.
<3> Vector registers `v16` through `v31` hold the output accumulators and
    are initialized to zero.
<4> With `EMUL_C=16`, one matrix instruction updates a single
    sixteen-register C tile.
<5> With `EMUL_C=8`, the sixteen output registers hold two C tiles.
<6> With `EMUL_C=4`, the output is decomposed into a 2-by-2 array of C
    tiles.
<7> With `EMUL_C=2`, the output is decomposed into a 4-by-2 array of C
    tiles.
<8> With `EMUL_C=1`, the output is decomposed into a 4-by-4 array of
