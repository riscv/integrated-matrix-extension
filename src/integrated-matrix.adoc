...
The IM extension imposes two more restrictions on `LAMBDA`. (An additional restriction will be introduced later.)
First, `LAMBDA`, and consequently `M`, must be a power of 2.
Second, we must have `M` ≥ `LAMBDA`.
...
[#ime-emulc-table]
.Table of legal implementation choices of `LAMBDA` for different values of `L` = (`VLEN` ÷ `SEW`). The entries in the table show the resulting multiplicity, |C| = `EMUL_C` of the vector register group holding tile C.
