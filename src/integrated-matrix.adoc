...
| `vqmmacc.vv vd, vs1, vs2[, v0.scale]` | 4 | SEW/4   | SEW | floating point and MX scaled A/B tiles
...
instructions (`vfwmmacc.vv`, `vqmmacc.vv`), not with `vfmmacc.vv`
...
three instructions (`vfmmacc.vv`, `vfwmmacc.vv`, `vqmmacc.vv`) are
available.
...
| vqmmacc.vv| FP64 (SEW=64)          | Zvvfp16fp64mm AND Zvvbf16fp64mm
|===
...
The K-dimension, tile-dimension formulae, MUL_C, and instruction-to-widening-factor mapping are the same as for the integer family (see <<zvvmm>>); the floating-point mnemonics are `vfmmacc.vv` (W=1), `vfwmmacc.vv` (W=2), and `vqmmacc.vv` (W=4).
...
** If `psm=2`, the exact products belonging to the group shall first be summed in infinite precision, and then rounded to the accumulator format according to the rounding mode specified in `frm`, thus producing the partial sum `S`.
...
[NOTE]
====
A Zvvm implementation can produce the exact same results as the analogous Zvtm implementation when operating on 16- and 8-bit inputs by setting `psm=1` and choosing a suitable architectural value of `G`: `G=2` for 16-bit inputs with 32-bit outputs, and `G=1` for 8-bit inputs with 32-bit outputs.
For input element widths of 32 bits or greater, compatibility with the analogous Zvtm instruction is obtained with `G=1` and `psm=2`, so that each group contains a single product and the partial sum `S` is that product rounded according to `frm`.
...
extension:
    ...
    vqmmacc.vv vd, vs1, vs2, v0.scale
...
multiply-accumulate instructions (`vfwmmacc.vv`, `vqmmacc.vv`, `vf8wmmacc.vv`)
...
* For widening variants (W=2 or W=4), use `vfwmmacc.vv`/`vqmmacc.vv` or
  `vwmmacc.vv`/`vqmmacc.vv`.
...
vfloat16m4_t __riscv_vfwmmacc_vv_f16m4_f8e5m2m1(vfloat16m4_t vd,
                                                  vfloat8e5m2m1_t vs1, vfloat8e5m2m1_t vs2, size_t vl);
/* vqmmacc.vv FP32 accum / OFP8 E5M2 inputs (altfmt_A=B=1 for 8-bit inputs): */
vfloat32m2_t __riscv_vqmmacc_vv_f32m2_f8e5m2m1(vfloat32m2_t vd,
                                                   vfloat8e5m2m1_t vs1, vfloat8e5m2m1_t vs2, size_t vl);
--
...
<<<
[#insns-vqmmacc_vv,reftext="Floating-Point Quad-Widening Matrix Multiply-Accumulate"]
==== vqmmacc.vv
...
vqmmacc.vv _vd_, _vs1_, _vs2_, v0.scale
...
{reg:[
    ...
    { bits: 5, name: 'vs2' },
    { bits: 1, name: 1 },
    { bits: 6, name: 0x16, attr: ['vqmmacc.vv'] }
]}
....
...
| vqmmacc | 16 | 4  | 0 | E2M1 | 0 | E2M1  | 0 | FP16  | Zvvofp4fp16mm    | Zvvxofp4fp16mm     | Zvvxnofp4fp16mm
| vqmmacc | 16 | 4  | 0 | E2M1 | 0 | E2M1  | 1 | BF16  | Zvvofp4bf16mm | Zvvxofp4bf16mm  | Zvvxnofp4bf16mm
| vqmmacc | 16 | 4  | 1 | —    | 1 | —     | × | —     | _reserved_     | —                | —
| vqmmacc | 32 | 8  | 0 | E4M3 | 0 | E4M3  | 0 | FP32  | Zvvofp8fp32mm    | Zvvxofp8fp32mm     | Zvvxnofp8fp32mm
| vqmmacc | 32 | 8  | 0 | E4M3 | 0 | E4M3  | 1 | —     | _reserved_     | —                 | —
| vqmmacc | 32 | 8  | 1 | E5M2 | 1 | E5M2  | 0 | FP32  | Zvvofp8fp32mm    | Zvvxofp8fp32mm     | Zvvxnofp8fp32mm
| vqmmacc | 32 | 8  | 1 | E5M2 | 1 | E5M2  | 1 | —     | _reserved_     | —                 | —
| vqmmacc | 32 | 8  | 0 | E4M3 | 1 | E5M2  | 0 | FP32  | Zvvofp8fp32mm    | Zvvxofp8fp32mm     | Zvvxnofp8fp32mm
| vqmmacc | 32 | 8  | 1 | E5M2 | 0 | E4M3  | 0 | FP32  | Zvvofp8fp32mm    | Zvvxofp8fp32mm     | Zvvxnofp8fp32mm
| vqmmacc | 64 | 16 | 0 | FP16 | 0 | FP16  | 0 | FP64  | Zvvfp16fp64mm       | —                 | —
| vqmmacc | 64 | 16 | 0 | FP16 | 0 | FP16  | 1 | —     | _reserved_     | —                 | —
| vqmmacc | 64 | 16 | 1 | BF16 | 1 | BF16  | 0 | FP64  | Zvvbf16fp64mm    | —                 | —
| vqmmacc | 64 | 16 | 1 | BF16 | 1 | BF16  | 1 | —     | _reserved_     | —                 | —
| vqmmacc | 64 | 16 | 0 | FP16 | 1 | BF16  | 0 | FP64  | Zvvfp16fp64mm, Zvvbf16fp64mm | —         | —
| vqmmacc | 64 | 16 | 1 | BF16 | 0 | FP16  | 0 | FP64  | Zvvfp16fp64mm, Zvvbf16fp64mm | —         | —
