...
* Within each group, the `G` sub-dot-products correspond to a dot product of two vectors with `G × W` elements. The result of that group dot product is reduced into a partial sum `S` according to an implementation-defined parameter `psm`.
