...
* Within each group, the G × W pairwise products are reduced into a partial sum `S`, according to an implementation-defined parameter `psm`.
...
** If `psm=1`, the exact products belonging to the group shall be accumulated in a bulk-normalized representation according to the RVBNA Algorithm, or in an equivalent representation producing the same numerical result.

* Once the partial sum `S` is computed, and before it is accumulated into the result C, it is rounded according to an implementation-defined parameter `rnd`.

** If `rnd=frm`, partial sum `S` is rounded to the C accumulator format using the rounding mode from `frm`.

** If `rnd=rto`, partial sum `S` is rounded to the C accumulator format using round-to-odd.

** If `rnd=xct`, partial sum `S` is kept in its exact representation, without rounding.
...
If `rnd=xct`, this is the only rounding step in the update of stem:[C_{i,j}] by a group of dot-products.

Supported values of `G`, `psm`, and `rnd` by an implementation may depend on `SEW`, `W`, and `lambda`.
An implementation must disclose all supported combinations, preferably by publishing a table of all supported (SEW, W, lambda) -> (G, psm, rnd) mappings.
The floating-point result of a matrix multiply-accumulate instruction is fully determined by this specification.

The resulting number of rounding operations per output element of C is [2](λ × LMUL) ÷ G, where the factor of 2 is present if the reduced partial sum is rounded before being added to the running value.
