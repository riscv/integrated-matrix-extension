...
*Dependence on `L`, and hence on `SEW`.* As shown in <<ime-emulc-examples>> and <<ime-emulc-examples-L32>>, different choices of λ for a given `L` = `VLEN` ÷ `SEW` lead to different shapes of the multiplier array. Since `VLEN` is fixed for an implementation, the flexibility in λ lets the same multiplier array shape be used for different `SEW`s. For example, if `VLEN`=512, a 16 × 16 array can be used with both (`SEW` = 32, λ = 1) and (`SEW` = 16, λ = 2). In general, a `M` × `M` multiplier array can be used by setting λ = `L` ÷ `M` (when legal). Narrower C elements (smaller `SEW`) raise the value of `L` and therefore require a larger λ to result in the same multiplier shape.
...
// A/B indices and epr are measured in input elements (EEW_A = SEW / W).
// Pass lambda * W to these helpers: each register holds lambda * W input
// elements per tile row. C continues to use the unscaled lambda.
