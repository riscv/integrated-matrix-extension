...
Compute intensity versus memory bandwidth. Each rank-LAMBDA update of the accumulator loads the A and B operands — `M`·`LAMBDA` elements each — and performs `M`·`M`·`LAMBDA` multiply-accumulates. The compute intensity, in MACs per loaded element, is therefore

    I = (M·M·LAMBDA) ÷ (2·M·LAMBDA) = M ÷ 2 = L ÷ (2LAMBDA).
...
The geometry of the multiplier and the tiles is defined by the new `lambda` (`LAMBDA`) field, which is encoded in 3 bits in the `vtype` CSR, in combination with the per-instruction widening factor `W` and the standard vector-configuration values `LMUL`, `SEW`, and `VLEN`.
