...
High-performance computing and machine learning workloads depend critically on general matrix multiplication over a wide range of data types and precisions.
...
By interpreting existing vector register groups as two-dimensional matrix tiles, the Zvvm family of Integrated Matrix extensions delivers high arithmetic density without introducing any new architected state.
...
The Zvvm family of Integrated Matrix extensions (Zvvmm, Zvvfmm, Zvvmtls, Zvvmttls) relies on the following extensions:
...
* <<#zvvmtls>>: two-dimensional load/store instructions for moving data between memory and vector register groups interpreted as matrix tiles
* <<#zvvmttls>>: two-dimensional transposing load/store instructions for moving data between memory and vector register groups interpreted as matrix tiles

==== Matrix tile multiplication geometry

The geometry of the multiplier and the tiles is defined by the new `vtype` field `lambda` (λ), encoded in 3 bits, in combination with the per-instruction widening factor `W` and the standard vector-configuration values `LMUL`, `SEW`, and `VLEN`.
...
The accumulator C always has element width `SEW`, regardless of `W`.

When `W = 1`, the instruction is _non-widening_: each input element has width `SEW`,
and no packing occurs.
...
* The _accumulator_ C is stored in a vector register group with element width SEW.
  The register-group multiplier of the C register group, denoted MUL_C, is the effective LMUL (EMUL) used for that group; it is determined by the tile geometry rather than by `vtype.LMUL`:
  ...
  MUL_C ∈ {1, 2, 4, 8, 16}.
  If MUL_C = 16, for example, the only allowed vector register indices are 0 and 16.
...
* The _input matrices_ A and B are stored in vector register groups with element width determined by the instruction:
  ...
  Fractional LMUL settings (LMUL < 1) are reserved when used with any IME instruction.

[#ime-geometry-fig]
.Geometry of matrix tiles and element ordering for 32 element vector registers and λ=4. Vector register groups are interpreted as 2D tiles. Vector element indices show the tile element order. (a) Non-widening case (W=1) with A, B, and C having the same SEW. (b) Double-widening case (W=2) with A and B at half the SEW of C, with two narrow input elements packed into each SEW-wide storage position.
...
| L_A = LMUL × VLEN ÷ (SEW ÷ W)         | Total elements in the A tile (or, equivalently, in the B^T^ tile) across LMUL registers.  L_A is a local helper used only in this table.
| K_eff = λ × W × LMUL                  | Effective shared-K dimension of the multiply-accumulate.
| M = N = L_A ÷ K_eff                   | Rows and columns of the square accumulator tile.
|===

NOTE: MUL_C = VLEN ÷ (SEW × λ²) is independent of both W and LMUL; the C register group always occupies MUL_C vector registers regardless of how A and B are configured.

The elements in the vector registers are contiguous along the K direction (with λ such elements per single vector register), as depicted in
<<ime-geometry-fig>>. Tile A and C elements are stored in row-major order while tile B^T^
elements are stored in column-major order.
Throughout this chapter, "the B tile" is used as shorthand for the
in-register tile that holds B^T^; it is the source of `vs2` for every
matrix multiply-accumulate instruction. This choice allows the implementation of the
...
The computational extensions in Table <<tbl-extensions>> can be combined in many ways,
...
as well as microscaling-specific extensions listed in <<mx-extensions-section>>.
